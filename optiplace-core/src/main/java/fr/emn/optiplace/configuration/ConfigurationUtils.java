...
public final class ConfigurationUtils {
	 ...
	 */
	public static ManagedElementSet<Node> usedNodes(Configuration cfg,
			EnumSet<State> wrt) {
	 ...
	 */
	public static ManagedElementSet<Node> unusedNodes(Configuration cfg,
			State wrt) {
		ManagedElementSet<Node> ns = new SimpleManagedElementSet<Node>();
		for (Node n : cfg.getOnlines()) {
			if (wrt == State.Runnings && cfg.getRunnings(n).isEmpty()) {
				ns.add(n);
			}
			if (wrt == State.Sleepings && cfg.getSleepings(n).isEmpty()) {
				ns.add(n);
			}
	 ...
	 */
	public static ManagedElementSet<Node> currentlyOverloadedNodes(
			Configuration cfg) {
		ManagedElementSet<Node> nodes = new SimpleManagedElementSet<Node>();
		if (cfg != null) {
			for (Node n : cfg.getOnlines()) {
				if (currentlyOverloaded(cfg, n)) {
					nodes.add(n);
				}
			}
		}
	...
	/**
	 * Indicates if a node is overloaded A node is currently overloaded if the
	 * total memory or CPU consumption of the virtual machines it hosts is
	 * greater than its memory or CPU capacity.
	 * 
	 * @return {@code true} if the node is overloaded. {@code false} if not or
	 *         if it is offline
	 */
	public static boolean currentlyOverloaded(Configuration cfg, Node n) {
		if (cfg.isOnline(n)) {
			int cpuCapa = n.getCoreCapacity() * n.getNbOfCores();
	...
	/**
	 * Indicates if a node will be saturated. A node will be saturated if the
	 * total memory or CPU demand of the virtual machines it hosts is greater
	 * than its memory or CPU capacity.
	 * 
	 * @return {@code true} if the node is saturated. {@code false} if not or if
	 *         it is offline
	 */
	public static boolean futureOverloaded(Configuration cfg, Node n) {
		if (cfg.isOnline(n)) {
			int cpuCapa = n.getCoreCapacity() * n.getNbOfCores();
			int memCapa = n.getMemoryCapacity();
			for (VirtualMachine vm : cfg.getRunnings(n)) {
	...
	public static ManagedElementSet<Node> futureOverloadedNodes(
			Configuration cfg) {
		ManagedElementSet<Node> nodes = new SimpleManagedElementSet<Node>();
		for (Node n : cfg.getOnlines()) {
			if (futureOverloaded(cfg, n)) {
				nodes.add(n);
			}
		}
		return nodes;
	...
	public static Configuration subConfiguration(Configuration cfg,
		...
		for (Node n : nodes) {
			if (cfg.isOffline(n)) {
				...
				/*
				 * if (!cpy.containsAll(runs)) { throw new
				 * ConfigurationsException(cfg, "VMs on node " + n.getName() +
				 * "(" + runs + ") does not only contains VMs in " + cpy); } if
				 * (!cpy.containsAll(sleeps)) { throw new
				 * ConfigurationsException(cfg, "VMs on node " + n.getName() +
				 * "(" + sleeps + ") does not only contains VMs in " + cpy); }
				 */
				cpy.removeAll(runs);
			 ...
			 * else { throw new ConfigurationsException(cfg, "Unknown node " +
			 * n.getName()); }
			 */
		}/*
		 * if (!cfg.getWaitings().isEmpty() &&
		 * !cfg.getWaitings().containsAll(cpy)) { throw new
		 * ConfigurationsException(cfg, "Waiting VMs (" + cfg.getWaitings() +
	 ...
	 */
	public static Configuration merge(List<Configuration> cfgs) {
	 ...
	 *             if configurations are conflicting between each other
	 */
	public static Configuration merge(Configuration... cfgs) {
		Configuration res = new SimpleConfiguration();
		for (Configuration c : cfgs) {
			// if (!Collections.disjoint(c.getAllVirtualMachines(),
			// res.getAllVirtualMachines())) {
			...
			for (Node n : c.getOfflines()) {
				// if (res.isOnline(n)) {
				// throw new ConfigurationsException(c,
				// "Conflicting state for node '" + n.getName() + "'");
				// }
				if (!res.isOffline(n)) {
					res.addOffline(n);
				}
			...
			for (Node n : c.getOnlines()) {
				...
				// throw new ConfigurationsException(c,
				// "Unable to place running '" + vm.getName()
				// + "' on '" + n.getName() + "'");
				// }
				// }
				...
				// throw new ConfigurationsException(c,
				// "Unable to place sleeping '" + vm.getName()
				// + "' on '" + n.getName() + "'");
				// }
				// }
			}
