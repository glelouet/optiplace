 ...
 * loads), using 2 invariants: 1) global load O(nbBins): the sum of the bin
 * loads is equal to the sum of the size items 2) knapsack on each bin O(nbItems
 * x nbBins): the bin load is bounded by the total size of the assigned items
 * and the total size of the assigned and candidate items. A simple improvement
 * of this latter upper bound is possible by activating the "big items"
 * optimization: big items are defined for a given bin such that no two of them
 * can be assigned simultaneously to the bin without violating its capacity. The
 * upper bound then becomes the total size of the assigned and candidate small
 * items + the maximal size among the big candidate items. With option (
 * {@code BigItemsPolicy.STATIC}) the list of big candidates is computed once
 ...
 */
public class FastBinPacking extends Propagator<Variable> {
	...
	//
	// /** The candidate items for each bin (possible but not required
	// assignments) */
	...
	//
	// /**
	// * "big items" optimization policies: 1) no optimization, or 2) static
	// * computation of the list of big candidates for each bin, or 3) dynamic
	...
	// public static final BigItemsPolicy DEFAULT_BIG_ITEMS_POLICY =
	// BigItemsPolicy.IGNORE;
	//
	// /**
	// * "big items" optimization: for each bin, the first "small" candidate item
	// * which can be hosted along with any bigger items.
	// */
	// private IStateInt[] firstSmall;
	//
	// /**
	// * "big items" optimization: the bins having a required big item (only for
	// * the STATIC policy).
	// */
	...
	// * the solver environment
	// * @param loads
	// * array of nbBins variables, each figuring the total size of the
	// * items assigned to it, usually initialized to [0, capacity]
	// * @param sizes
	// * array of nbItems CONSTANTS in DECREASING order, each figuring
	// * the size of i-th biggest item
	// * @param bins
	...
	// }
	// sumISizes = sum;
	// bTLoads = new IStateInt[nbBins];
	// bRLoads = new IStateInt[nbBins];
	// bigItemsPolicy = bigItemsFlag;
	// if (bigItemsPolicy != BigItemsPolicy.IGNORE) {
	// firstSmall = new IStateInt[nbBins];
	// if (bigItemsPolicy == BigItemsPolicy.STATIC) {
	// bigRequired = env.makeBitSet(nbBins);
	// }
	// }
	// }
	//
	...
	// // "
	// // + Arrays.asList(itemSizesDecreasing) + ";" +
	...
	// for (int i = candidates[bin].nextSetBit(0); i >= 0; i = candidates[bin]
	// .nextSetBit(i + 1)) {
	// s.append(i).append(' ');
	// }
	...
	// @Override
	// /**
	// * initialize the internal data: availableBins, candidates,
	// binRequiredLoads, binTotalLoads, sumLoadInf, sumLoadSup
	...
	// loadsHaveChanged = env.makeBool(false);
	//
	...
	// @Override
	// /**
	// * propagate 1) globally: sumItemSizes == sumBinLoads 2) on each bin:
	// sumAssignedItemSizes == binLoad
	// * rule 1.0: if sumSizes < sumBinLoadInfs or sumSizes > sumBinLoadSups then
	// fail
	// * rule 1.1, for each bin: sumItemSizes - sumOtherBinLoadSups <= binLoad <=
	// sumItemSizes - sumOtherBinLoadInfs
	// * rule 2.0, for each bin: binRequiredLoad <= binLoad <= binTotalLoad
	// * rule 2.1, for each bin and candidate item: if binRequiredLoad + itemSize
	// > binLoadSup then remove item from bin
	// * rule 2.2, for each bin and candidate item: if binTotalLoad - itemSize <
	// binLoadInf then pack item into bin
	// * with "big items" optimization, the last rule is not valid for big items
	// but can be replaced by:
	// * rule 2.3: if smallItemSizes < binLoadInf then remove big candidates with
	// size < binLoadInf-smallItemSizes
	// * and update binLoadInf as binRequiredLoad + the size of the smallest big
	// remaining candidate
	// */
	...
	// }
	// noFixPoint = false;
	// for (int b = availableBins.nextSetBit(0); b >= 0; b = availableBins
	// .nextSetBit(b + 1)) {
	// noFixPoint |= filterLoadInf(
	// b,
	// Math.max(
	// bRLoads[b].get(),
	...
	//
	// /**
	// * recompute the sum of the min/max loads only if at least one variable
	// * bound has been updated outside the constraint
	// */
	...
	// // @Override
	// // /**
	// // * on loads variables: update sum of the bin load LBs + UBs, check it
	// // against sumItemSizes, then delay propagation
	// // */
	...
	// // @Override
	// // /**
	// // * on loads variables: update sum of the bin load LBs, check it against
	// // sumItemSizes, then delay propagation
	// // */
	...
	// // @Override
	// // /**
	// // * on loads variables: update sum of the bin load UBs, check it against
	// // sumItemSizes, then delay propagation
	// // */
	...
	//
	// /**
	// * on bins variables: propagate the removal of item-to-bins assignments. 1)
	...
	// */
	// @Override
	// public void awakeOnRemovals(int iIdx, DisposableIntIterator deltaDomain)
	// throws ContradictionException {
	//
	// try {
	...
	//
	// /**
	// * synchronize the internal data when an item is assigned to a bin: remove
	...
	//
	// /**
	// * propagate the knapsack constraint on a given bin: 1) remove the candidate
	// * items bigger than the remaining free space (when binRequiredLoad +
	// * itemSize > binLoadSup) 2) pack the candidate items necessary to reach the
	// * load LB (when binTotalLoad - itemSize < binLoadInf). code optimized
	...
	// boolean ret = false;
	// for (int item = candidates[bin].nextSetBit(0); item >= 0; item =
	// candidates[bin]
	// .nextSetBit(item + 1)) {
	...
	//
	// /**
	// * Check the consistency of the required and candidate loads with regards to
	...
	// loads[b].pretty() + " required=" + bRLoads[b].get()
	// + " expected=" + rs[b]);
	// check = false;
	// }
	// if (!doBigItemsOpt() && rs[b] + cs[b] != bTLoads[b].get()) {
	// ChocoLogging.getBranchingLogger().warning(
	// loads[b].pretty() + " total=" + bTLoads[b].get()
	// + " expected=" + (rs[b] + cs[b]));
	// check = false;
	// }
	...
	// ChocoLogging.getBranchingLogger().warning(
	// "Sum Load LB = " + this.sumLoadInf.get() + " expected ="
	// + sumLoadInf);
	// check = false;
	// }
	// if (this.sumLoadSup.get() != sumLoadSup) {
	// ChocoLogging.getBranchingLogger().warning(
	// "Sum Load UB = " + this.sumLoadSup.get() + " expected ="
	// + sumLoadSup);
	// check = false;
	// }
	...
	// if (!check) {
	// for (int b = 0; b < rs.length; b++) {
	// System.out.println(loads[b].pretty() + " required="
	// + bRLoads[b].get() + " (" + rs[b] + ") total="
	// + bTLoads[b].get() + " (" + (rs[b] + cs[b]) + ")");
	// }
	// System.out.println("Sum Load LB = " + this.sumLoadInf.get() + " ("
	// + sumLoadInf + ")");
	...
	// ChocoLogging.getBranchingLogger().warning(
	// "candidate i '" + i + "' for bin '" + b + ": "
	// + candidates[b].get(i) + " expected: "
	// + bs[b].get(i));
	// ChocoLogging.getBranchingLogger().warning(
	// "candidates for b: " + prettyCandidates(b));
	// ChocoLogging.flushLogs();
	// return false;
	// }
	// }
	...
	//
	// /**
	// * initialize the big candidates list and compute the candidate load
	...
	// * bin index
	// * @param firstB
	// * index of the current first candidate
	// * @param firstS
	// * index of the previous first small candidate
	// * @return the candidate load decrement = maxBigCandidateSizes -
	// * sumBigNewCandidateSizes <= 0
	// */
	...
	// return bigItemsPolicy == BigItemsPolicy.STATIC ? bigItemsAssignStatic(
	// item, bin) : bigItemsAssignDynamic(item, bin);
	// }
	//
	// /**
	// * update binTotalLoad if the new assigned item is a big candidate: if item
	...
	// assert !bigRequired.get(bin); // no 2 big items can be assigned to
	// // bin
	// bigRequired.set(bin);
	// int firstB = candidates[bin].nextSetBit(0);
	// if (firstB < item) {
	...
	// * @return the required + candidate load decrement <=0
	// * @throws ContradictionException
	// * on the bins variables
	// */
	// private int bigItemsAssignDynamic(int item, int bin)
	// throws ContradictionException {
	// int firstB = candidates[bin].nextSetBit(0);
	// int firstS = firstSmall[bin].get();
	// int decLoad = -(firstB < firstS ? item < firstB
	// ? iSizes[item]
	// : iSizes[firstB] : 0);
	//
	// if (item < firstS) { // the new assigned item is a big item => remove
	// // all
	// // big candidates
	// decLoad += iSizes[item]; // itemSize is now in binRequiredLoad
	// for (; firstB >= 0 && firstB < firstS; firstB = candidates[bin]
	// .nextSetBit(firstB + 1)) {
	...
	// assert bigItemsPolicy != BigItemsPolicy.IGNORE;
	// return bigItemsPolicy == BigItemsPolicy.STATIC ? bigItemsRemoveStatic(
	// item, bin) : bigItemsRemoveDynamic(item, bin);
	// }
	//
	// /**
	// * update binTotalLoad if the new removed item is a big candidate: if a big
	// * item is already assigned => the max term iSizes[bigRequired] remains
	// * unchanged (and iSizes[item] must be readded) else if item was the only
	// * big candidate => the max term iSizes[item] disappear else if item was the
	...
	// firstS = candidates[bin].nextSetBit(firstS + 1);
	// } else if (candidates[bin].nextSetBit(firstS + 1) < 0) { // no lonely
	// // small
	// // candidate
	// if (firstB < firstS) {
	// decLoad -= iSizes[firstS];
