 ...
 */
public class Var2ValSelector implements IntValueSelector, VariableSelector<IntVar> {
     ...
     *
     * @param vars
     *            variables to try to instantiate to given value
     * @param vals
     *            at least as many vals as in vars
     */
	...
	throw new UnsupportedOperationException("can not set variable " + var + ", know vars are "
		+ Arrays.asList(vars));
    }

    @Override
    public IntVar getVariable(IntVar[] variables) {
	// assert variables == vars : "expected " + Arrays.asList(vars) +
	// ", got "
	// + Arrays.asList(variables);
	for (int i = 0; i < vars.length; i++) {
	    if (!vars[i].isInstantiated()
		    && (vals == null && vars[i].contains(val) || vals != null && vars[i].contains(vals[i]))) {
		return vars[i];
	    }
	}
	return null;
