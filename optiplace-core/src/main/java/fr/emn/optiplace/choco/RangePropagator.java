...
/**
 * A propagator to ensure an index idx refers to the range a variable X is in.
 * The ranges are stored as an array of int, with the int at the index i being
 * the max value of the range at this index.<br />
 * eg., the int provided at the position 0 of the ranges is the maximum value of
 * the range at position 0, and the range at position 1 goes from ranges[0]+1 to
 * ranges[1]
 *
...
public class RangePropagator extends Propagator<IntVar> {
	...
	private static final org.slf4j.Logger logger = org.slf4j.LoggerFactory
			.getLogger(RangePropagator.class);
	 ...
	 * @param ranges
	 * the range limits, They must all be ordered(for any i, ranges[i]<ranges[i+1]
	 * )
	 */
	public RangePropagator(IntVar X, IntVar index, int... ranges) {
		super(new IntVar[] { X, index }, PropagatorPriority.LINEAR, true);
		assert testIncreasing(ranges) : "variables for range are not increasing "
	...
	public void propagate(int evtmask) throws ContradictionException {
		...
		for (int i = 0; i < getNbRanges(); i++) {
			...
			if (idx.contains(i)) {
				...
				if (!x.contains(lb)
						&& (x.nextValue(lb) > ub || x.nextValue(lb) == Integer.MAX_VALUE)) {
	...
	public ESat isEntailed() {
		...
		if (index.isInstantiated()) {
			...
			// none of them is instantiated : we check for each range available if x
			// can belong to it. it is FALSe iff no range remaining in idx can match x
