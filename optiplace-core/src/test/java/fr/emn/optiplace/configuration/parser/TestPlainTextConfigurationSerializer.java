...
import fr.emn.optiplace.configuration.parser.ConfigurationSerializerException;
import fr.emn.optiplace.configuration.parser.FileConfigurationSerializer;
import fr.emn.optiplace.configuration.parser.PlainTextConfigurationSerializer;

/**
 * Unit tests for PlainTextConfigurationSerialized.
 * 
 * @author Fabien Hermenier
 */
@Test(groups = {"unit"})
public class TestPlainTextConfigurationSerializer {
	...
	public void test() {
		...
		for (int i = 1; i <= 20; i++) {
			VirtualMachine vm = new SimpleVirtualMachine("VM" + i, i, i + 1,
					i + 2);
			vm.setCPUDemand(i + 3);
			vm.setCPUMax(i + 7);
		...
		FileConfigurationSerializer s = PlainTextConfigurationSerializer
				.getInstance();
		File tmpF;
		try {
