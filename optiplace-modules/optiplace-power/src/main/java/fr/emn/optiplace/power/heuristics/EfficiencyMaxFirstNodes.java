...
public class EfficiencyMaxFirstNodes {
  ...
  .getLogger(EfficiencyMaxFirstNodes.class);
  ...
  /** @param cd the powerview to retrieve data from
   * @param fallbackResourceName */
  public EfficiencyMaxFirstNodes(PowerData cd,
  ...
  public List<AbstractStrategy<? extends Variable>> getHeuristics(
    ...
    if (secondaryResource != null
        && rp.getResourcesHandlers().get(secondaryResource) != null) {
      rp.getResourcesHandlers().get(secondaryResource).getSpecs();
    }
    List<AbstractStrategy<? extends Variable>> ret = new ArrayList<>();
    Node[] sortedNodes = sortNodesByEfficiencyMax(rp, fallbackResource);
    ArrayList<IntVar> vmsHosters = extractVMsWithComp(sortedNodes,
        rp, inNodeComparator);
   ...
   * capacity */
  public Node[] sortNodesByEfficiencyMax(IReconfigurationProblem rp,
      ResourceSpecification fallbackResource) {
    Map<Node, Double> efficiencies = new HashMap<>();
    ResourceSpecification cpur = rp.getResourcesHandlers().get("CPU")
        .getSpecs();
    ...
    ArrayList<Entry<Node, Double>> entries = new ArrayList<Map.Entry<Node, Double>>(
        efficiencies.entrySet());
   ...
   * @param sortedNodes the nodes to take the VMs from, in an order respected by
   * VMs
   * @param rp the problem to get the vars of the VMs from.
   * @param comp the comparator to sort the VMs inside a node
   * @return the variables of the hosters of the VMs, sorted by node, using the
   * comparator inside a node desc. */
  public static ArrayList<IntVar> extractVMsWithComp(Node[] sortedNodes,
      IReconfigurationProblem rp, Comparator<VM> comp) {
    ArrayList<IntVar> vmsHosters = new ArrayList<IntVar>();
    for (int i = sortedNodes.length - 1; i >= 0; i--) {
      Node n = sortedNodes[i];
      ArrayList<VM> vms = new ArrayList<VM>(rp
          .getSourceConfiguration().getHosted(n).collect(Collectors.toList()));
  ...
  public static class NodeEfficiencyComparator implements
  Comparator<Entry<Node, Double>> {
    ...
    public int compare(Entry<Node, Double> o1, Entry<Node, Double> o2) {
      ...
      if (fallbackComparison != null) {
        double memDiff = fallbackComparison.getCapacity(o2.getKey())
            - fallbackComparison.getCapacity(o1.getKey());
        if (memDiff != 0) {
          return (int) memDiff;
        }
      }
