...
public abstract class ActionModel {
	 ...
	 * @param solver
	 *            the solver
	 * @return an Action. May be null the it does not generate any action for
	 *         the Defaultentropy.plan.ReconfigurationResult
	 */
	public abstract List<Action> getDefinedAction(ReconfigurationProblem solver);

	/**
	 * Put the result of applying this solved action into a configuration.
	 * 
	 * @param solver
	 *            the solver to get the solution
	 * @param cfg
	 *            the configuration to populate
	 * @return true if the action modify the configuration
	 */
	public abstract boolean putResult(ReconfigurationProblem solver,
			Configuration cfg);
