...
/**
 * A constraint to maintains bounds for all the incoming and the outgoing slices
 * for a node.
 * <p/>
 * TODO: Adapt to have a single constraint for all the nodes, not one per node
 * (unjustified) TODO: Reduce the memory footprint by avoiding to create an
 * array of key for each propagate?
 * 
...
public class PlanMySlices extends AbstractLargeIntSConstraint {
	...
	/**
	 * The moment the consuming slices ends. Same order as the hosting
	 * variables.
	 */
	private IntVar[] cEnds;

	/**
	 * The CPU height for each consuming slice. Same order as the hosting
	 * variables.
	 */
	private int[] cCPUHeights;

	/**
	 * The Memory height for each consuming slice. Same order as the hosting
	 * variables.
	 */
	...
	/*
	 * The moment the demanding slices ends. Same order as the hosting
	 * variables.
	 */
	private IntVar[] dStarts;

	/**
	 * The CPU height for each demanding slice. Same order as the hosting
	 * variable.
	 */
	private int[] dCPUHeights;

	/**
	 * The memory heighr for each demanding slice. Same order as the hosting
	 * variable.
	 */
	 ...
	 * @param cHosters
	 *            The hoster variable for all the consuming slices
	 * @param cCPUHeights
	 *            the CPU height for the consuming slices (same order as
	 *            cHosters)
	 * @param cMemHeights
	 *            the memory height for the consuming slices (same order as
	 *            cHosters)
	 * @param cEnds
	 *            the moments the consuming slices will end (same order as
	 *            cHosters)
	 * @param dHosters
	 *            the hoster variable for all the demanding slices
	 * @param dCPUHeights
	 *            the CPU height for the demanding slices (same order as
	 *            dHosters)
	 * @param dMemHeights
	 *            the memory height for the demanding slices (same order as
	 *            dHosters)
	 * @param dStarts
	 *            the moments the demanding slices will starts (same order as
	 *            dHosters)
	 */
	public PlanMySlices(IEnvironment env, int me, int capacityCPU,
	...
	/**
	 * Update the structure of the constraints. Set the moments for the LB and
	 * the UB.
	 * 
	 * @throws ContradictionException
	 *             if an instantiation is not consistent
	 */
	...
	/**
	 * Translation for a relatives resources changes to an absolute free
	 * resources.
	 * 
	 * @param changes
	 *            the map that indicates the free CPU variation
	 * @param sortedMoments
	 *            the different moments sorted in ascending order
	 */
	private static void toAbsoluteFreeResources(TIntIntHashMap changes,
			int[] sortedMoments) {
	...
	public void awake() throws ContradictionException {
		...
		for (IntVar dHoster : dHosters) {
			if (dHoster.isInstantiated()) {
				if (me == DEBUG) {
					ChocoLogging.getBranchingLogger().finest(
							"Already instantiated:" + dHoster);
				}
		...
		if (me == DEBUG) {
			ChocoLogging.getBranchingLogger().finest(
					"me= " + me + " cpuCapa=" + capacityCPU + ", memCapa="
							+ capacityMem);
			ChocoLogging
					.getBranchingLogger()
					.finest(toInstantiate.get()
							+ " placement variable to instantiate before activation");
		}
	}
	...
	public void awakeOnInst(int idx) throws ContradictionException {
		if (idx < dHosters.length) {
			toInstantiate.set(toInstantiate.get() - 1);
			if (dHosters[idx].getVal() == me && me == DEBUG) {
				ChocoLogging.getBranchingLogger().finest(
						me + "-- " + dHosters[idx].getName()
								+ " on me. Still waiting for "
								+ toInstantiate.get());
			}
		}
		constAwake(false);
	...
	public boolean isSatisfied(int[] vals) {
		...
		// A hashmap to save the changes (relatives to the previous moment) in
		...
		for (int i = 0; i < dHostersVals.length; i++) {
			if (dHostersVals[i] == me) {
				cpuChanges.put(dStartsVals[i], cpuChanges.get(dStartsVals[i])
						- dCPUHeights[i]);
				memChanges.put(dStartsVals[i], memChanges.get(dStartsVals[i])
						- dMemHeights[i]);
			}
		}
		...
		for (int i = 0; i < cHostersVals.length; i++) {
			if (cHostersVals[i] == me) {
				cpuChanges.put(cEndsVals[i], cpuChanges.get(cEndsVals[i])
						+ cCPUHeights[i]);
				memChanges.put(cEndsVals[i], memChanges.get(cEndsVals[i])
						+ cMemHeights[i]);
		...
		if (me == DEBUG) {
			...
			ChocoLogging.getBranchingLogger().finest(
					me + " currentFreeCPU=" + currentFreeCPU);
			ChocoLogging.getBranchingLogger().finest(
					me + " currentFreeMem=" + currentFreeMem);
		...
		for (int i = 0; i < cpuChanges.keys().length; i++) {
			...
			if (currentFreeCPU < 0 || currentFreeMem < 0) {
				ChocoLogging.getMainLogger().severe(
						me + " at moment " + i + ": freeCPU=" + currentFreeCPU
	...
	private void computeProfiles() {
		...
		for (int i = out.nextSetBit(0); i >= 0; i = out.nextSetBit(i + 1)) {
			...
			if (associatedToDSliceOnCurrentNode(i)
				...
				if (me == DEBUG) {
					ChocoLogging.getBranchingLogger().finest(
							me + " " + cEnds[i].pretty()
									+ " decreasing or non-associated ("
									+ dStarts[revAssociations[i]].pretty()
									+ "?)");
				}
		...
		if (me == DEBUG) {
			ChocoLogging.getBranchingLogger().finest(
					"---" + me + "--- startup=(" + startupFreeCPU + "; "
							+ startupFreeMem + ") init=(" + capacityCPU + "; "
							+ capacityMem + ")");
			for (int i = in.nextSetBit(0); i >= 0; i = in.nextSetBit(i + 1)) {
				ChocoLogging.getBranchingLogger().finest(
						(dStarts[i].isInstantiated() ? "!" : "?") + " "
								+ dStarts[i].pretty() + " " + dCPUHeights[i]
								+ " " + dMemHeights[i]);
			}

			for (int i = out.nextSetBit(0); i >= 0; i = out.nextSetBit(i + 1)) {
				ChocoLogging.getBranchingLogger().finest(
						(cEnds[i].isInstantiated() ? "!" : "?") + " "
								+ cEnds[i].pretty() + " " + cCPUHeights[i]
								+ " " + cMemHeights[i]);
			}
			ChocoLogging.getBranchingLogger().finest("---");
			...
			ChocoLogging.getBranchingLogger().finest(
					"profileMin="
							+ prettyProfile(sortedMinProfile, profileMinCPU,
									profileMinMem));
			ChocoLogging.getBranchingLogger().finest(
					"profileMax="
							+ prettyProfile(sortedMaxProfile, profileMaxCPU,
									profileMaxMem));
		}
	}
	...
	private boolean associatedToDSliceOnCurrentNode(int cSlice) {
		if (revAssociations[cSlice] != NO_ASSOCIATIONS
			...
			if (me == DEBUG) {
				ChocoLogging.getBranchingLogger().finest(
						me + " " + cEnds[cSlice].getName() + " with "
								+ dStarts[revAssociations[cSlice]]);
			}
			return true;
		}
		return false;
	...
	private boolean associatedToCSliceOnCurrentNode(int dSlice) {
		if (associations[dSlice] != NO_ASSOCIATIONS
			...
			if (me == DEBUG) {
				ChocoLogging.getBranchingLogger().finest(
						me + " " + dStarts[dSlice].getName() + " with "
								+ cEnds[associations[dSlice]]);
			}
			return true;
		}
		return false;
	...
	private static String prettyProfile(int[] ascMoments,
			TIntIntHashMap cpuProfile, TIntIntHashMap memProfile) {
	...
	private void checkInvariant() throws ContradictionException {
		for (int t : sortedMinProfile) {
			if (profileMinCPU.get(t) > capacityCPU
					|| profileMinMem.get(t) > capacityMem) {
				// if (me == DEBUG) {
				ChocoLogging.getBranchingLogger().finest(
						me
								+ ": Invalid profile at moment "
								+ t
								+ " - "
								+ prettyProfile(sortedMinProfile,
										profileMinCPU, profileMinMem));
				// }
				fail();
			}
		}
	...
	private void updateDStartsInf() throws ContradictionException {
		for (int i = in.nextSetBit(0); i >= 0; i = in.nextSetBit(i + 1)) {
			if (!dStarts[i].isInstantiated()
					&& !associatedToCSliceOnCurrentNode(i)) {
				...
				for (int x = sortedMinProfile.length - 1; x >= 0; x--) {
					...
					if (t <= dStarts[i].getUB()
							&& (profileMinCPU.get(prevT) + dCpu > capacityCPU || profileMinMem
									.get(prevT) + dMem > capacityMem)) {
						lastT = t;
						break;
					}
				}
	...
	private void updateDStartsSup() throws ContradictionException {
		...
		for (int i = sortedMaxProfile.length - 1; i >= 0; i--) {
			...
			if (profileMaxCPU.get(t) <= capacityCPU
					&& profileMaxMem.get(t) <= capacityMem) {
				lastSup = t;
			} else {
				break;
			}
		...
		if (lastSup != -1) {
			for (int i = in.nextSetBit(0); i >= 0; i = in.nextSetBit(i + 1)) {
				if (!dStarts[i].isInstantiated()
						&& !associatedToCSliceOnCurrentNode(i)
						&& dStarts[i].getUB() > lastSup) {
					int s = Math.max(dStarts[i].getLB(), lastSup);
					if (me == DEBUG) {
						ChocoLogging.getBranchingLogger().finest(
								me + ": " + dStarts[i].pretty() + " ub=" + s
										+ ");");
					}
					dStarts[i].setSup(s);
				}
	...
	private void updateCEndsSup() throws ContradictionException {
		for (int i = out.nextSetBit(0); i >= 0; i = out.nextSetBit(i + 1)) {
			if (!cEnds[i].isInstantiated()
					&& !associatedToDSliceOnCurrentNode(i)) {
				...
				for (int t : sortedMinProfile) {
					if (t >= cEnds[i].getUB()) {
						break;
							...
							&& (profileMinCPU.get(t) + cCpu > capacityCPU || profileMinMem
									.get(t) + cMem > capacityMem)) {
						lastT = t;
						break;
					}
				}
