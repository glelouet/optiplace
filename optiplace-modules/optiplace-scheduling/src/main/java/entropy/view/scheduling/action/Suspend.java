...
public class Suspend extends MovingVirtualMachineAction {
	 ...
	 * 
	 * @param vm
	 *            the virtual machine to suspend
	 * @param n
	...
	public String toString() {
		return new StringBuilder("suspend(").append(name).append(',')
				.append(getDestination().getName()).append(')').toString();

	}

	/**
	 * Apply the action by putting the virtual machine into sleep on its
	 * destination node for a specified configuration.
	 * 
	 * @param c
	 *            the configuration
	 */
	 ...
	 * configuration. The source node must be online and running the VM while
	 * the destination node must be online and the VM should be sleep on it
	 * 
	 * @param src
	...
	public boolean isCompatibleWith(Configuration src, Configuration dst) {
		if (!src.isOnline(getHost()) || !src.isRunning(getVirtualMachine())
				|| !src.getLocation(getVirtualMachine()).equals(getHost())) {
			return false;
		}
		return (!dst.isOnline(getDestination())
				|| !dst.isSleeping(getVirtualMachine()) || !dst.getLocation(
				getVirtualMachine()).equals(getDestination()));
	}
	 ...
	 * @return true if ref is an instanceof Suspend and if both instance involve
	...
	public boolean insertIntoGraph(TimedExecutionGraph g) {
		return g.getUnlockings(getDestination()).add(this);
	}
