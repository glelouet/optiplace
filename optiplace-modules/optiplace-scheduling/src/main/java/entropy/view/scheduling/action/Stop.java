...
public class Stop extends VirtualMachineAction {
	...
	/**
	 * Apply the action by removing the virtual machine from a specified
	 * configuration.
	 * 
	 * @param c
	 *            the configuration
	...
	public boolean isCompatibleWith(Configuration src) {
		return (src.isOnline(getHost()) && src.isRunning(getVirtualMachine()) && src
				.getLocation(getVirtualMachine()).equals(getHost()));
	}

	/**
	 * Check the compatibility of the action with a source and a destination
	 * configuration. The VM must be initially running and not be a part of the
	...
	public boolean isCompatibleWith(Configuration src, Configuration dst) {
		return (!src.isRunning(getVirtualMachine())
				...
				|| dst.isWaiting(getVirtualMachine()) || dst
					.isSleeping(getVirtualMachine()));
	}
	...
	public String toString() {
		return new StringBuilder("stop(").append(name).append(',')
	...
	public boolean insertIntoGraph(TimedExecutionGraph g) {
		return g.getUnlockings(getHost()).add(this);
	}
	 ...
	 * @return true if ref is an instanceof Stop and if both instance involve
