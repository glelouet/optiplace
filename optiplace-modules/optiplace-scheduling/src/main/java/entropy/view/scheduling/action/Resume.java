...
/**
 * An action to resume a VirtualMachine on an online node. The state of the
 * virtual machine comes to "sleeping" to "running".
 * 
...
public class Resume extends MovingVirtualMachineAction {
	 ...
	 * 
	 * @param vm
	 *            the virtual machine to resume
	 * @param from
	 *            the node that currently host the virtual machine image
	 * @param to
	...
	public String toString() {
		return new StringBuilder("resume(").append(name).append(',')
	 ...
	 * hosting node must be online and hosting the sleeping the virtual machine
	 * 
	 * @param src
	...
	public boolean isCompatibleWith(Configuration src) {
		return (src.isOnline(getHost()) && src.isSleeping(getVirtualMachine())
	 ...
	 * configuration. Destination node must be online and running the VM on the
	 * destination configuration while VM must be sleeping on the hosting node
	 * on the source configuration The VM must be waiting in the source
	 * configuration.
	 * 
	 * @param src
	 ...
	 * @return true if ref is an instanceof Resume and if both instance involve
	 *         the same virtual machine and the same nodes
	 */
	@Override
