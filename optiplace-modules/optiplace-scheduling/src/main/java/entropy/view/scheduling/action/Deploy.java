...
/**
 * A node action to deploy an hosting platform on a node that is compatible
 * with. The node must be online and not host any virtual machines. At the end
 * of the action, the node is online with the new hosting platform ready to host
 * VMs.
 * 
...
public class Deploy extends NodeAction {
	 ...
	 *            the involved node
	 * @param p
	 *            the platform to deploy on the node. This platform must be
	 ...
	 * @param st
	 *            the moment the action finishes
	 */
	...
	public boolean isCompatibleWith(Configuration src) {
		...
		return n2.getAvailablePlatforms().contains(p)
				&& src.getRunnings(n2).isEmpty()
				&& src.getSleepings(n2).isEmpty();
	}
	...
	public boolean isCompatibleWith(Configuration src, Configuration dst) {
		return isCompatibleWith(src)
				...
				&& dst.getAllNodes().get(getNode().getName())
						.getCurrentPlatform().equals(p);
	}
	...
	public String toString() {
		StringBuilder b = new StringBuilder("deploy(").append(getNode()
				.getName());
