...
/**
 * A graph to represent dependencies between the actions to perform on a graph.
 * A managed element in the graph represents a potential lock. A lock may be the
 * source and the target of outgoing or incoming actions, respectively.
 * 
...
public class TimedExecutionGraph {
	...
	/**
	 * Extracts all the dependencies of the graph. A dependency occurs when the
	 * start moment of a lockable action is greater or equals to the finish
	 * moment of unlocker action that involve the same managed element.
	 * 
	...
	public Set<Dependencies> extractDependencies() {
		...
		for (Map.Entry<ManagedElement, Set<Action>> e : this.lockable
				.entrySet()) {
	...
	/**
	 * Get the lockable actions on an element. A lockable action is an action
	 * that may have to wait for something on the element in order to be
	 * executed (most of the time, just resources)
	 * 
	 * @param n
	 *            the element
	...
	/**
	 * Get the unlocking actions on a element An unlocking action is an action
	 * that participate in the dependencies destruction. when it is performed
	 * 
	 * @param n
	 *            the element
	 * @return a list, may be empty
	...
	public String toEventAgenda() {
		...
		for (Dependencies dep : deps) {
			if (!events.containsKey(dep.getUnsatisfiedDependencies())) {
				events.put(dep.getUnsatisfiedDependencies(),
						new HashSet<Action>());
			}
