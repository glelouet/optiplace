...
/**
 * An action to model the boot of a node. The action is modeled with a consuming
 * action.
 * 
...
public class BootNodeActionModel extends NodeActionModel {
	...
	/**
	 * Make a new action. The following constraint is added into the model:
	 * <ul>
	 ...
	 */
	public BootNodeActionModel(SchedulingView model, Node n, int d) {
		super(n);
		ReconfigurationProblem problem = model.getProblem();
		cSlice = new ConsumingSlice(model, "boot(" + n.getName() + ")", n,
				n.getCPUCapacity(), n.getMemoryCapacity(), d);
