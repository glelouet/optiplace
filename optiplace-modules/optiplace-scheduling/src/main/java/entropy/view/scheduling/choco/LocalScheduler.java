...
public class LocalScheduler {
	...
	public LocalScheduler(int me, int capacityCPU, int capacityMem,
			int[] cCPUHeights, int[] cMemHeights, IntVar[] cEnds,
			BitSet outs, int[] dCPUHeights, int[] dMemHeights,
			IntVar[] dStarts, IStateBitSet in, int[] assocs,
			int[] revAssocs) {
		associations = assocs;
		this.me = me;
		...
		revAssociations = revAssocs;/*
									 * new int[cCPUHeights.length]; for (int i =
									 * 0; i < revAssociations.length; i++) {
									 * revAssociations[i] = NO_ASSOCIATIONS; }
									 * for (int i = 0; i < associations.length;
									 * i++) { if (associations[i] !=
									 * NO_ASSOCIATIONS) {
