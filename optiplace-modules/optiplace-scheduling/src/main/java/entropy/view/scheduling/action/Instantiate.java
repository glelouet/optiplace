...
/**
 * Create a VM that will be able to be booted after. The action may be a
 * standalone action, or attached to a boot action in the same plan.
 * <p/>
 * If it is standalone, then the VM is not a part of the source configuration
 * will be in the waiting state at the end of the reconfiguration. Otherwise,
 * the VM is not a part of the source configuration and will be in the running
 * state at the end of the reconfiguration.
 * 
...
public class Instantiate extends VirtualMachineAction {
	...
	public boolean isCompatibleWith(Configuration src) {
		return (src.isRunning(getVirtualMachine()) || !src
				.getAllVirtualMachines().contains(getVirtualMachine()));
	}

	@Override
	public boolean isCompatibleWith(Configuration src, Configuration dst) {
		return (!src.getAllVirtualMachines().contains(getVirtualMachine()) && (dst
				.getWaitings().contains(getVirtualMachine()) || dst
				.getRunnings().contains(getVirtualMachine())));
	}
	...
	public boolean apply(Configuration c) {
		...
		// Because a run action may have insert the VM before. Not nice but
		// still...
	 ...
	 * @return true if ref is an instance of Instantiate and if both instance
	 *         involve the same virtual machine
	 */
	@Override
	...
	public String toString() {
		return new StringBuilder("instantiate(").append(name).append(')')
				.toString();
	}
