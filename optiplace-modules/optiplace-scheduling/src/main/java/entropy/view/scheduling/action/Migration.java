...
public class Migration extends MovingVirtualMachineAction {
	 ...
	 * @return true if ref is an instanceof Migration and if both instance
	 *         involve the same virtual machine and the same nodes
	 */
	@Override
	...
	public String toString() {
		return new StringBuilder("migrate(").append(name).append(',')
				.append(this.getHost().getName()).append(',')
				.append(this.getDestination().getName()).append(')').toString();
	}

	/**
	 * Apply the action by running the virtual machine to its destination node
	 ...
	 * hosting node must be online and running the virtual machine
	 * 
	 * @param src
	...
	public boolean isCompatibleWith(Configuration src) {
		return (src.isOnline(getHost()) && src.isRunning(getVirtualMachine())
				&& src.getLocation(getVirtualMachine()).equals(getHost()) && src
					.isOnline(getDestination()));
	}

	/**
	 * Check the compatibility of the action with a source and a destination
	 * configuration. Hosting node must be online on the source configuration
	 * and run the VM. Destination node must be online in the destination
	 * configuration and run the VM.
	 * 
	 * @param src
	...
	/**
	 * Insert the action as an incoming action for the destination node and as
	 * an outgoing action for the current hosting node.
	 * 
	 * @param g
	 *            the graph to use
	...
	public boolean insertIntoGraph(TimedExecutionGraph g) {
		return g.getLockables(getDestination()).add(this)
				&& g.getUnlockings(getHost()).add(this);
	}
