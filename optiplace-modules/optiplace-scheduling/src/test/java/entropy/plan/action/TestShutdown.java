...
public class TestShutdown {
	...
	public void testGets() {
		Shutdown s = new Shutdown(new SimpleNode("N1", 1, 1, 1));
	...
	public void testEquals() {
		Shutdown s = new Shutdown(new SimpleNode("N1", 1, 1, 1));
		Shutdown s2 = new Shutdown(new SimpleNode("N1", 1, 1, 1));
		Shutdown s3 = new Shutdown(new SimpleNode("N2", 1, 1, 1));
	...
	public void testIsIncompatibleWith(Shutdown s) {
		...
		c.setSleepOn(new SimpleVirtualMachine("VM10", 1, 2, 3), n3);
	...
	public Object[][] getWrongShutdowns() {
		return new Object[][]{{new Shutdown(new SimpleNode("n1", 1, 2, 3))}, // host
				// running
				// VM
				{new Shutdown(new SimpleNode("n2", 1, 2, 3))}, // bad state
				{new Shutdown(new SimpleNode("n3", 1, 2, 3))} // host sleeping
		// VM
		};
	}
