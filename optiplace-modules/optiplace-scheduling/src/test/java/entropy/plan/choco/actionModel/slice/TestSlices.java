...
import entropy.view.scheduling.actionModel.slice.Slices;
...
public class TestSlices {
	...
	private Slice[] buildSlices() {
		...
		for (int i = 0; i < size; i++) {
			slices[i] = new Slice("s" + i, s.createBoundIntVar("h" + i, 0, 5),
					s.createTaskVar("toto", s.makeConstantIntVar(0),
							s.makeConstantIntVar(5), s.makeConstantIntVar(5)),
					i, i * 2);
		}
		return slices;
	}
	...
	public void testExtractHosters() {
		...
		IntVar[] hosters = Slices.extractHosters(slices);
