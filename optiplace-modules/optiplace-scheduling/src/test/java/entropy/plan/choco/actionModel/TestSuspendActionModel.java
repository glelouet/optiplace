...
public class TestSuspendActionModel {
	...
	/**
	 * Check the creation of a SuspendActionModel solve the problem and get the
	 * resulting action
	 */
	public void testSolvingWithSuspendActionCreation() {
		...
		VirtualMachineActionModel a = view.getAssociatedAction(vm1);
		...
		Assert.assertEquals(vma.getConsumingSlice().getCPUheight(),
				vm1.getCPUConsumption());
		Assert.assertEquals(vma.getConsumingSlice().getMemoryheight(),
				vm1.getMemoryConsumption());
