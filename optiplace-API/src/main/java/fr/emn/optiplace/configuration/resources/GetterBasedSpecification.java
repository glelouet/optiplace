...
public abstract class GetterBasedSpecification implements ResourceSpecification {
   ...
   * @param map the map to add data inside, or null to create one
   * @param res the list of resource specifications. can be null or empty
  ...
  public double getLoad(Configuration cfg) {
    int vmsUse = cfg.getVMs().collect(Collectors.summingInt(this::getUse));
    int nodesCapa = cfg.getNodes().collect(
        Collectors.summingInt(this::getCapacity));
    return ((double) vmsUse) / nodesCapa;
  }

  @Override
  public double getUse(Configuration cfg, Node n) {
    return (double) cfg.getHosted(n).collect(
        Collectors.summingInt(this::getUse));
  }
