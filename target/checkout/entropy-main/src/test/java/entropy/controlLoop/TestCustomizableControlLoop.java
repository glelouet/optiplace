...
import entropy.vjob.builder.plasma.MockVirtualMachineBuilder;
...
public class TestCustomizableControlLoop {
  ...
  /* public void testsWithNoVJobs() { try { Configuration src =
   * TestHelper.readConfiguration(RESOURCES_DIR + "emptyConf.txt");
   * MockConfigurationAdapter mockConfAdapter = new MockConfigurationAdapter();
   * mockConfAdapter.useConfiguration(src); MockVirtualMachineBuilder vmBuilder
   * = new MockVirtualMachineBuilder(); DurationEvaluator eval = new
   * MockDurationEvaluator(5, 2, 2, 3, 6, 3, 1, 1); MockDriverFactory
   * mockDrvFactory = new MockDriverFactory(); VJobBuilder b = new
   * VJobBuilder(new VJobElementBuilder(vmBuilder), null); VJobsPool queue = new
   * FCFSPersistentQueue(b, new File(RESOURCES_DIR + "emptyQueue/"));
   * CustomizableControlLoop loop = new CustomizableControlLoop(new
   * Monitor(mockConfAdapter), queue, b, eval, new
   * TimedReconfigurationExecuter(mockDrvFactory)); loop.setAssignTimeout(30);
   * loop.setPlanTimeout(30); loop.setMasterVJobFile(RESOURCES_DIR +
   * "parts.txt"); loop.runLoop();
   * Assert.assertEquals(mockDrvFactory.getNbActions(), 0);
   * //Thread.sleep(30000); } catch (Exception e) { Assert.fail(e.getMessage(),
   * e); } } */

  /** 2 VJobs without constraints. Just ensure the configuration will be
   * maintained as viable. */
  public void testWithNoConstraintedVJobs() {
    try {
      ...
      DurationEvaluator eval = new MockDurationEvaluator(10, 5, 2, 2, 3, 6, 3,
          1, 1);
      MockDriverFactory mockDrvFactory = new MockDriverFactory();
      DefaultVJobBuilderFactory vjobF = new DefaultVJobBuilderFactory();
      // vjobF.add(new PlasmaVJobBuilder(new DefaultVJobElementBuilder(new
      // MockVirtualMachineBuilder()), null));

      VJobsPool queue = new FCFSPersistentQueue(vjobF, new File(RESOURCES_DIR
          + "2VJobs/"));
      CustomizableControlLoop loop = new CustomizableControlLoop(new Monitor(
          mockConfAdapter), queue, vjobF, eval,
          new TimedReconfigurationExecuter(mockDrvFactory));
  ...
  public void testConfLighter() {
    ...
    Assert.assertEquals(
        ConfigurationUtils.currentlyOverloadedNodes(src).size(), 0);
  }
