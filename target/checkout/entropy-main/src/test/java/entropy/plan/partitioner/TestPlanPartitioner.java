...
public class TestPlanPartitioner {
  ...
  public void testSimpleFence() {
    ...
    for (int i = 0; i < apps.length; i++) {
      ...
      for (int j = 0; j < 10; j++) {
        VirtualMachine vm = new SimpleVirtualMachine("VM" + (i * 10 + j + 1),
            1, 1, 1);
        apps[i].add(vm);
        cfg.addWaiting(vm);
      }
    ...
    Assert.assertTrue(ps.get(0).getNodes().equals(parts[0])
        && ps.get(0).getVirtualMachines().containsAll(apps[0])
        && ps.get(0).getVirtualMachines().containsAll(apps[4]));
    Assert.assertTrue(ps.get(1).getNodes().equals(parts[1])
        && ps.get(1).getVirtualMachines().containsAll(apps[1]));
    Assert.assertTrue(ps.get(2).getNodes().equals(parts[2])
        && ps.get(2).getVirtualMachines().containsAll(apps[2]));
    Assert.assertTrue(ps.get(3).getNodes().equals(parts[3])
