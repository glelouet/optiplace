...
/** Tests XenRPCClient. At this moment, tests require an hypervisor :/
 * @author Fabien Hermenier */
public class TestXenRPCClient {

  // ALL tests commented because they depends on testAuth method and this
  // method use Assert.fail();
  ...
  // @Test(groups = {"integration:hypervisor"}, sequential = true,
  // dependsOnMethods = {"testAuth"})
  public void testGetUUID() {
    try {
      XenRpcClient client = new XenRpcClient("b217.info.emn.fr", 9363);
      client.login("root", "");
      Assert.assertEquals(client.getUUID("Domain-0"),
  ...
  public void testGetNameFromUUID() {
    try {
      ...
      client.login("root", "");
      Assert.assertEquals(
          client.getNameFromUUID("00000000-0000-0000-0000-000000000000"),
          "Domain-0");
      client.logout();
    }
    catch (Exception e) {
  ...
  public void testSaveAndRestore() {
    try {
      ...
      Assert.assertTrue(client.save("tinyLenny1", "/tmp/tinyLenny1.state",
          false));
  ...
  public void testGets() {
    try {
      ...
      Assert.assertEquals(client.getRemotePort(),
          XenRpcClient.DEFAULT_XEN_API_PORT);
