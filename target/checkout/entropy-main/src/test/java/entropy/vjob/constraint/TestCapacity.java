...
public class TestCapacity {
  ...
  /** Basic test, all the VMs are online, so as the nodes Move some VMs from one
  ...
  public void test1() {
    ...
    try {
      ...
      Assert.assertEquals(c.getMaximumCapacity(), 5);
      vjobs.add(v);
      entropy.plan.ReconfigurationResult p = plan.compute(cfg, cfg.getRunnings(),
          cfg.getWaitings(), cfg.getSleepings(),
  ...
  /** Trickier test. while some VMs in the other partition will have to move to
   * the restricted partition. Last, one VM that was suspended on the restricted
  ...
  public void test2() {
    ...
    for (int i = 1; i <= 6; i++) {
      if (i <= 3) {
        Node n = new SimpleNode("N" + i, 1, 10, 10);
        cfg.addOnline(n);
      } else {
        Node n = new SimpleNode("N" + i, 1, 5, 5);
        cfg.addOnline(n);
      }
    }
    ...
    try {
      ...
      entropy.plan.ReconfigurationResult p = plan.compute(cfg,
          cfg.getAllVirtualMachines(), cfg.getWaitings(),
          new SimpleManagedElementSet<VirtualMachine>(),
          new SimpleManagedElementSet<VirtualMachine>(), futureOnlines,
          futureOfflines, vjobs);
      System.err.println(p);
      p.getDestination();
      // 1 resume action
      // 1 migration to the restricted partition
      // 4 migrations to liberate the restricted partition ( 3 + 1 as a
      // side effect of the incoming migration)
