...
public class TestConstraintsCatalogBuilderFromProperties {
  ...
  private static final String RESOURCES_ROOT = "src/test/resources/entropy/vjob/builder/plasma/TestConstraintsCatalogBuilderFromProperties.";

  public void testWithNoConstraints() {
    PropertiesHelper props = TestHelper.readEntropyProperties(RESOURCES_ROOT
        + "noConstraints.txt");
    ConstraintsCatalogBuilder builder = new ConstraintsCatalogBuilderFromProperties(
        props);
    try {
  ...
  @Test(expectedExceptions = { ConstraintsCalalogBuilderException.class })
  public void testWithMissingLocation()
  ...
  public void testWithDefaultProperties() {
    try {
      PropertiesHelper props = new PropertiesHelper(
          "config/plasmaVJobs.properties");
      ConstraintsCatalogBuilder builder = new ConstraintsCatalogBuilderFromProperties(
          props);
      ConstraintsCatalog c = builder.build();
      Assert.assertEquals(c.getAvailableConstraints().size(), 8);
    }
    catch (Exception e) {
      Assert.fail(e.getMessage(), e);
    }
