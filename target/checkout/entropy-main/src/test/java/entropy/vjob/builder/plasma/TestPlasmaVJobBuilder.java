...
public class TestPlasmaVJobBuilder {
  ...
  /** Make a Simple VJobBuilder. Configuration is based on nodes in the pastel
   * cluster. 50 VMs are available constraint Ban, OneOf and lSpread in the
   * catalog. */
  private PlasmaVJobBuilder makeBuilder() {
    ...
    for (int i = 1; i <= 3; i++) {
      cfg.addOnline(new SimpleNode("pastel-nfs" + i + ".b217.home", 1, 1, 1));
    }
    cfg.addOnline(new SimpleNode("pastel-frontend.b217.home", 1, 1, 1));

    // Add some virtual machines
    for (int i = 1; i <= 50; i++) {
      cfg.addWaiting(new SimpleVirtualMachine("VM" + i, 1, 1, 1));
    }
  ...
  public void testWithProlog() {
    PlasmaVJobBuilder b = makeBuilder();
    try {
      PlasmaVJob infra = b.build("pastel", new File(RESOURCES_ROOT
          + "pastel.txt"));
      b.setProlog(infra);
      PlasmaVJob admin = b.build("admin", new File(RESOURCES_ROOT
          + "admin_vjob.txt"));
  ...
  public void testWebAppVJob() {
    ...
    try {
      PlasmaVJob infra = b.build("pastel", new File(RESOURCES_ROOT
          + "pastel.txt"));
      b.setProlog(infra);
      PlasmaVJob webapp = b.build("webapp", new File(RESOURCES_ROOT
          + "webapp.txt"));
