...
public class TestAmong {
  ...
  public void testWithOneGroup() {
    ...
    try {
      ...
      Assert.assertTrue(!res.getRunnings(n1).contains(vm1)
          && !res.getRunnings(n2).contains(vm1));
      Assert.assertTrue(!res.getRunnings(n1).contains(vm2)
          && !res.getRunnings(n2).contains(vm2));
  ...
  public void testWithTwoGroup() {
    ...
    try {
      ...
      Assert.assertTrue(res.getRunnings(n3).contains(vm1)
          || res.getRunnings(n3).contains(vm2)
          || res.getRunnings(n3).contains(vm3));
      Assert.assertTrue(res.getRunnings(n4).contains(vm1)
          || res.getRunnings(n4).contains(vm2)
          || res.getRunnings(n4).contains(vm3));
      for (PlacementConstraint c : v.getConstraints()) {
        if (!c.isSatisfied(res)) {
          Assert.fail(c + " is not satisfied");
        }
  ...
  public void testIsSatisfied() {
    ...
    vms.add(vm2);
    vms.add(vm3);
    Assert.assertFalse(new Among(cfg.getAllVirtualMachines(), grps)
        .isSatisfied(cfg));
    vms.remove(vm4);
