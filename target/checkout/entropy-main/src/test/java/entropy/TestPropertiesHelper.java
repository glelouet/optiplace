...
public class TestPropertiesHelper {
  ...
  public void testGetRequiredPropertyWithExistantProperty() {
    try {
      PropertiesHelper props = new PropertiesHelper(RESOURCES_DIR
          + "testGetRequiredProperty.txt");
  ...
  public void testGetRequiredPropertyWithIncorrectFormat()
  ...
  public void testGetRequiredPropertyWithUnknownProperty()
  ...
  public void testGetOptionalProperty() {
    PropertiesHelper props = TestHelper.readEntropyProperties(RESOURCES_DIR
        + "testGetRequiredProperty.txt");

    // Property exists, must return the current value
    Assert
        .assertEquals(props.getOptionalProperty("oneProp", "myDefault"), "ok");
    // Unknown property, must return the default value
    try {
      Assert.assertEquals(props.getOptionalProperty("unknown", 2), 2);
      Assert.assertEquals(props.getOptionalProperty("unknown", "myDefault"),
          "myDefault");
    }
  ...
  @Test(expectedExceptions = { WrongPropertyTypeException.class })
  public void testGetOptionalPropertyWithWrongFormat()
      throws WrongPropertyTypeException {
    PropertiesHelper props = TestHelper.readEntropyProperties(RESOURCES_DIR
        + "testGetRequiredProperty.txt");
