...
public class DiffTree extends VJobTree {
  ...
  public Content go(VJobTree parent) {
    ...
    if (l.type() != r.type()) {
      return ignoreError("Type mismatch: right operand is not a "
          + l.type().toString());
    } else {
      if (l.type() == Content.Type.vmset || l.type() == Content.Type.nodeset) {
        return new Content(l.type(), new SetsDifference((VJobSet) l.content(),
            (VJobSet) r.content()));
      } else if (l.type() == Content.Type.multinodesets
          || l.type() == Content.Type.multivmsets) {
        return new Content(l.type(), new MultiSetsDifference(
            (VJobMultiSet) l.content(), (VJobMultiSet) r.content()));
      }
