...
public class ExplodedSetTree extends VJobTree {
  ...
  public Content go(VJobTree parent) {
    if (getChildCount() > 0) {
      ...
      switch (t0.type()) {
        ...
        default:
          errors.append(((VJobTree) getChild(0)).token,
              "a set cannot be composed of " + t0.type().toString());
          return Content.ignore;
      }
    } else {
  ...
  private Content makeMultiSet(Content.Type expect, Content.Type produce) {
    ExplodedMultiSet<ManagedElement> set = new ExplodedMultiSet<ManagedElement>();
    for (int i = 0; i < getChildCount(); i++) {
      Content tx = ((VJobTree) getChild(i)).go(this);
      if (!tx.type().equals(expect)) {
        return ignoreError("Type mismatch for "
            + ((ManagedElement) tx.content()).getName() + " expected " + expect
            + " but was " + tx.type());
      } else {
        set.add((VJobSet<ManagedElement>) tx.content());
      }
    }
  ...
  private Content makeSet(Content.Type expect, Content.Type produce) {
    ExplodedSet<ManagedElement> set = new ExplodedSet<ManagedElement>();
    for (int i = 0; i < getChildCount(); i++) {
      Content tx = ((VJobTree) getChild(i)).go(this);
      if (!tx.type().equals(expect)) {
        return ignoreError("Type mismatch for "
