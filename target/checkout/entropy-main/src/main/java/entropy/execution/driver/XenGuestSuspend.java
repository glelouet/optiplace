...
/** A driver to perform suspend of Xen virtual machines. This driver can be
 * configured using the properties loaded with {@link entropy.PropertiesHelper}.
 * The following table sum the available propertie.
 * <p/>
 * <table>
 * <tr>
...
public class XenGuestSuspend extends XenAPIDriver {
  ...
  public XenGuestSuspend(Suspend action, PropertiesHelper props)
      throws PropertiesHelperException {
    super(action, props);
    stateFilesLocation = props
        .getRequiredProperty(STATE_FILE_LOCATION_PROPERTY);
    sp = action;
  }
  ...
  public void execute() throws DriverException {
    try {
      XenRpcClient client = new XenRpcClient(sp.getHost().getName(), getPort());
      client.login(getUsername(), getPassword());
      client.save(sp.getVirtualMachine().getName(), stateFilesLocation + "/"
          + sp.getVirtualMachine().getName() + STATE_FILE_PREFIX, false);
      client.logout();
    }
    catch (IOException e) {
