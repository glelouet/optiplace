...
/** Abstract class to provide a Driver that execute an action with a SSH command
 * on a remote host.
...
public abstract class SSHDriver extends Driver {
   ...
   * the driver */
  public SSHDriver(Action a, PropertiesHelper props)
      throws PropertiesHelperException {
    super(a);
    username = props.getOptionalProperty(
        PropertiesHelper.AUTH_USERNAME_PROPERTY,
        System.getProperty("user.name"));
    identityFile = props
        .getRequiredProperty(PropertiesHelper.AUTH_PRIVATE_KEY_PROPERTY);
  }
  ...
  public void execute() throws DriverException {
    try {
      SSHExec ssh = new SSHExec(getRemoteHostname(), SSHExec.DEFAULT_PORT,
          username);

      ssh.useIdentity(identityFile);
      String realCmd = getCommandToExecute();
      int ret = ssh.executeCommand(realCmd);
      if (ret != 0) {
        throw new DriverException(this, "Error while execute the command '"
            + realCmd + "': return " + ret);
      }
