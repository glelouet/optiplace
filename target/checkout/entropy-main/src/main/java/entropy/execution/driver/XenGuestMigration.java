...
/** Driver to perform the migration of a VirtualMachine managed by a Xen
 * Hypervisor. This driver can be configured using properties loaded into
...
public class XenGuestMigration extends XenAPIDriver {
  ...
  public XenGuestMigration(Migration action, PropertiesHelper props)
      throws PropertiesHelperException {
    super(action, props);
    mig = action;
    relocationPort = props.getRequiredPropertyAsInt(RELOCATION_PORT_PROPERTY);
  }
  ...
  public void execute() throws DriverException {
    XenRpcClient client;
    try {
      client = new XenRpcClient(mig.getHost().getName(), getPort());
      client.login(getUsername(), getPassword());
      client.migrate(mig.getVirtualMachine().getName(), mig.getDestination()
          .getName(), true, relocationPort);
      client.logout();
    }
