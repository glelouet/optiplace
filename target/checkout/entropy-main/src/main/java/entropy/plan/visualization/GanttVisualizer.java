...
/** Visualize a reconfiguration plan as a agenda. Image can be stored as a JPEG
 * or a PNG file.
...
public class GanttVisualizer implements PlanVisualizer {
  ...
  public boolean buildVisualization(entropy.plan.ReconfigurationResult plan) {
    ...
    if (parent != null && !parent.exists() && !parent.mkdirs()) {
      ReconfigurationPlanner.logger.error("Unable to create '" + out + "'");
      return false;
    }
    final TaskSeriesCollection collection = new TaskSeriesCollection();
    TaskSeries ts = new TaskSeries("actions");
    for (Action action : plan) {
      Task t = new Task(action.toString(), new SimpleTimePeriod(
          action.getStartMoment(), action.getFinishMoment()));
      ts.add(t);
    }
    collection.add(ts);
