...
/** A Factory to select the serializer to use when reading or writing a
 * configuration file. The serializer is selected using the file extension.
 * <p/>
 * Supported Serializer:
 * <ul>
 * <li>{@link entropy.configuration.parser.PlainTextConfigurationSerializer}:
 * File extension must be {@value #PLAIN_TEXT_EXTENSION}.</li>
 * <li>{@link entropy.configuration.parser.ProtobufConfigurationSerializer}:
 * File extension must be {@value #PROTOBUF_EXTENSION}.</li>
 * </ul>
...
public final class FileTimedReconfigurationPlanSerializerFactory {
  ...
  /** Read a plan from a file. The serializer to use is inferred using the file
   * extension.
   * @param path the path to the file.
   * @return the parsed plan or {@code null} if no serializer is associated to
   * the file extension
   * @throws java.io.IOException if an error occurred while reading the file
   * @throws TimedReconfigurationPlanSerializerException if an error occurred
   * while parsing the file */
  public entropy.plan.ReconfigurationResult read(String path) throws IOException,
  ...
  /** Write a plan to a file. The serializer to use is inferred from the file
   * extension.
   * @param plan the configuration to write
   * @param path the path to the file
   * @return {@code true} if the operation succeed, {@code false} if no
   * serializer is associated to the file extension
   * @throws java.io.IOException if an error occurred while writing the file */
  public boolean write(entropy.plan.ReconfigurationResult plan, String path)
      throws IOException {
    FileTimedReconfigurationPlanSerializer s = getSerializer(path);
    if (s == null) {
      return false;
