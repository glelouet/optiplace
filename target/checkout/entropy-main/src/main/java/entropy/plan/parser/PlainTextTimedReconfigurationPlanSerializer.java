...
/** A serializer to store a plan in a human readable plain text format. TODO:
 * Need some tests.
 * @author Fabien Hermenier */
public class PlainTextTimedReconfigurationPlanSerializer extends
  ...
  @Override
  public entropy.plan.ReconfigurationResult unSerialize(InputStream i)
      throws IOException, TimedReconfigurationPlanSerializerException {
    BufferedReader in;
    entropy.plan.ReconfigurationResult p = null;
    try {
      in = new BufferedReader(new InputStreamReader(i));
      Configuration cfg = PlainTextConfigurationSerializer.getInstance()
          .unSerialize(in);
      p = new Defaultentropy.plan.ReconfigurationResult(cfg);
      ...
      while (line != null) {
        ...
        if (toks.length != 2) {
          throw new TimedReconfigurationPlanSerializerException(
              "Non viable action line:" + line);
        }
        String[] moments = toks[0].split(":");
        if (moments.length != 2) {
          throw new TimedReconfigurationPlanSerializerException(
              "Incorrect time interval:" + toks[0]);
        }
        try {
          ...
          if (id.equals(MIGRATION)) {
            a = new Migration(vms.get(params[0]), ns.get(params[1]),
                ns.get(params[2]), st, ed);
          } else if (id.equals(STOP)) {
            a = new Stop(vms.get(params[0]), ns.get(params[1]), st, ed);
          } else if (id.equals(RUN)) {
            a = new Run(vms.get(params[0]), ns.get(params[1]), st, ed);
          } else if (id.equals(PAUSE)) {
            a = new Pause(vms.get(params[0]), ns.get(params[1]), st, ed);
          } else if (id.equals(UNPAUSE)) {
            a = new UnPause(vms.get(params[0]), ns.get(params[1]), st, ed);
          } else if (id.equals(SUSPEND)) {
            a = new Suspend(vms.get(params[0]), ns.get(params[1]),
                ns.get(params[2]), st, ed);
          } else if (id.equals(RESUME)) {
            a = new Resume(vms.get(params[0]), ns.get(params[1]),
                ns.get(params[2]), st, ed);
          } else if (id.equals(STARTUP)) {
            a = new Startup(ns.get(params[0]), st, ed);
          } else if (id.equals(SHUTDOWN)) {
            a = new Shutdown(ns.get(params[0]), st, ed);
          } else if (id.equals(INSTANTIATE)) {
            a = new Instantiate(vms.get(params[0]), st, ed);
          } else if (id.equals(DEPLOY)) {
            a = new Deploy(ns.get(params[0]), params[1], st, ed);
          } else {
            throw new TimedReconfigurationPlanSerializerException(
          ...
          if (!p.add(a)) {
            throw new TimedReconfigurationPlanSerializerException(
                "Unable to add action '" + a + "'");
          }

        }
        catch (NumberFormatException e) {
          throw new TimedReconfigurationPlanSerializerException(
              "Incorrect time interval for " + toks[0] + ": " + e.getMessage());
        }
        line = in.readLine();
      }
    }
  ...
  @Override
  public void serialize(entropy.plan.ReconfigurationResult plan, OutputStream o)
      throws IOException {
    try {
      out = new BufferedWriter(new OutputStreamWriter(o));
      PlainTextConfigurationSerializer.getInstance().serialize(
          plan.getSource(), o);
      // Separator
  ...
  @Override
  public void serialize(Instantiate a) throws IOException {
  ...
  @Override
  public void serialize(Deploy a) throws IOException {
    out.write(DEPLOY);
    out.write("(");
    out.write(a.getNode().getName());
    out.write(")");
