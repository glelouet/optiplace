...
public class PlanThread extends Thread {
  ...
  private entropy.plan.ReconfigurationResult plan;
  ...
  public PlanThread(Partition part, DurationEvaluator eval, Configuration src,
      ...
      ManagedElementSet<Node> off) throws ConfigurationsException,
      PlanException {

    // Divide the original configuration
    cfg = ConfigurationUtils.subConfiguration(src, part.getVirtualMachines(),
        part.getNodes());
  ...
  public void run() {
    try {
      ...
      // Copy statistics to liberate memory more easily (no more
  ...
  public entropy.plan.ReconfigurationResult getResultingPlan() {
    return plan;
  }
