...
/** A Monitor instance is used to retrieve the current configuration of an
 * architecture. It uses a ConfigurationAdapter to extract the configuration and
 * a set of ConfigurationSampler to simplify the configuration if it is
 * specified.
...
public class Monitor {
  ...
  public Monitor(ConfigurationAdapter a) {
    adapter = a;
    samplers = new LinkedList<ConfigurationSampler>();
  }

  /** Attach a configuration sampler. It will be added at the end of the list of
   * samplers.
  ...
  /** Get the configuration. First the configuration is extracted using the
   * ConfigurationAdapter then, each attached sampler is used to simplify the
   * configuration.
   * @return a configuration
   * @throws MonitoringException if an error occurs during the extraction of the
   * configuration */
  public Configuration getConfiguration() throws MonitoringException {
    Configuration tmp = adapter.extractConfiguration();
    for (ConfigurationSampler sampler : samplers) {
