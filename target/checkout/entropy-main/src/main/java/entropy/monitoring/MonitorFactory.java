...
public class MonitorFactory {
  ...
  public static final String MEM_SAMPLER_SAMPLER_IMPL_SIZE = MEM_SAMPLER + "."
      + MEM_SAMPLER_SAMPLER_IMPL + ".size";
  ...
  public static final String CPU_SAMPLER_SAMPLER_IMPL_SIZE = CPU_SAMPLER + "."
      + CPU_SAMPLER_SAMPLER_IMPL + ".size";

  public static final String CPU_SAMPLER_BURNING_IMPL = "CPUBurning";

  public static final String CPU_SAMPLER_BURNING_IMPL_SIZE = CPU_SAMPLER + "."
      + CPU_SAMPLER_BURNING_IMPL + ".threshold";
   ...
   * @throws entropy.PropertiesHelperException if an error occurred while
   * configuring the adapter
   * @throws MonitorFactoryException If an error occurred during the
   * instantiation */
  public Monitor createMonitor() throws PropertiesHelperException,
      MonitorFactoryException {
   ...
   * @throws MonitorFactoryException If an error occurred during the
   * instantiation */
  private ConfigurationAdapter createConfAdapter()
      throws PropertiesHelperException, MonitorFactoryException {
    String impl = properties.getRequiredProperty(ADAPTER_IMPL_PROPERTY);
    try {
      Class<?> cl = getClass().getClassLoader().loadClass(impl);
      ConfigurationAdapterBuilder b = (ConfigurationAdapterBuilder) cl
          .newInstance();
      return b.build(b.getDistributionConfigurationFile());
    }
    catch (ClassNotFoundException e) {
      throw new MonitorFactoryException("Unable to locate implementation '"
          + impl + "'" + e.getMessage(), e);
    }
    catch (InstantiationException e) {
      throw new MonitorFactoryException("Unable to instantiate '" + impl + "'"
          + e.getMessage(), e);
    }
    catch (IllegalAccessException e) {
      throw new MonitorFactoryException("Error while accessing to '" + impl
          + "':" + e.getMessage(), e);
    }
  }
  ...
  private void attachNodeLists(Monitor mon) throws IOException,
      PropertiesHelperException, MonitorFactoryException {
    if (properties.isDefined(NODES_BL_PROPERTY)
        && properties.isDefined(NODES_WL_PROPERTY)) {
      throw new MonitorFactoryException(
          "Having a blacklist and a whitelist is not supported");
    }

    if (properties.isDefined(NODES_BL_PROPERTY)) {
      mon.getConfigurationAdapter().setNodesBlackList(
          readNodes(properties.getRequiredProperty(NODES_BL_PROPERTY)));
    } else if (properties.isDefined(NODES_WL_PROPERTY)) {
      mon.getConfigurationAdapter().setNodesWhiteList(
          readNodes(properties.getRequiredProperty(NODES_WL_PROPERTY)));
    }
   ...
   * instantiation
   * @throws IOException if an error occurred while reading the nodes file */
  private static List<String> readNodes(String file) throws IOException,
      MonitorFactoryException {
   ...
   * @throws entropy.PropertiesHelperException if an error occurred while
   * configuring the driver */
  private void attachSamplers(Monitor mon) throws PropertiesHelperException,
      MonitorFactoryException {
    if (properties.isDefined(MEM_SAMPLER_IMPL)) {
      String impl = properties.getRequiredProperty(MEM_SAMPLER_IMPL);
      if (impl.equals(MEM_SAMPLER_SAMPLER_IMPL)) {
        mon.attach(new MemorySampler(properties
            .getRequiredPropertyAsInt(MEM_SAMPLER_SAMPLER_IMPL_SIZE)));
      } else {
        throw new MonitorFactoryException("Unknown implementation '" + impl
            + "' for property '" + MEM_SAMPLER_IMPL + "'");
      }
    }
    if (properties.isDefined(CPU_SAMPLER_IMPL)) {
      String impl = properties.getRequiredProperty(CPU_SAMPLER_IMPL);
      if (impl.equals(CPU_SAMPLER_SAMPLER_IMPL)) {
        mon.attach(new CPUSampler(properties
            .getRequiredPropertyAsInt(CPU_SAMPLER_SAMPLER_IMPL_SIZE)));
      } else if (impl.equals(CPU_SAMPLER_BURNING_IMPL)) {
        mon.attach(new CPUBurningVMsSampler(properties
            .getRequiredPropertyAsInt(CPU_SAMPLER_BURNING_IMPL_SIZE)));
      } else {
        throw new MonitorFactoryException("Unknown implementation '" + impl
            + "' for property '" + CPU_SAMPLER_IMPL + "'");
      }
    }
  }
}
