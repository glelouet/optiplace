 ...
 * <p/>
 * <p/>
 * For nodes, CPU capacity is rounded to the nearest lower integer that divide
 * the parameter. For VMs, CPU consumption is rounded to the nearest upper
 * integer that divide the parameter.
...
public class CPUSampler implements ConfigurationSampler {
  ...
  public Configuration sample(Configuration cfg) {
    ...
    for (Node n : cfg.getOnlines()) {
      ...
      copy.setCPUCapacity((int) Math.floor(n.getCPUCapacity()
          / (double) divider));
    ...
    for (VirtualMachine vm : cfg.getRunnings()) {
      ...
      copy.setCPUConsumption((int) Math.ceil(vm.getCPUConsumption()
          / (double) divider));
