...
/** A SAX2 Parser to create a configuration from a XML Stream. The XML schema
 * corresponds to a GMetad XML output. The GMeta XML must contains some specific
 * metrics in order to identify containers (nodes that host vitual machines) and
 * virtual machines.
 * <p/>
 * <p/>
 * A HOST is considered as a container if all the following metrics are defined.
 * The CPU_CAPACITY of a node by computing the frequency with the number of
 * physical CPUs. The result is rounded using ConfigurationAdapter.round(...).
 * <p/>
 * <table>
 * <tr>
 * <th>Identifier</th>
 ...
 * <br/>
 * <br/>
 * In order to identify a supposed virtual machine, the HOST must contains the
 * following metrics. All of them are standard gmond metrics.
 * <table>
 * <tr>
 * <th>Identifier</th>
 * <th>Description</th>
 ...
 * </table>
 * <p/>
 * In order to be added into the configuration, each HOST considered as a
 * virtual machine must be hosted on one host that is considered as a container.
...
public class GangliaMetaXMLParser extends DefaultHandler {
  ...
  /** Metric that indicates the identifier of the driver used to migrate virtual
   * machines. */
  public static final String METRIC_CONTAINER_MIGRATION_DRIVER = "container.driver.migration";

  /** Metric that indicates the identifier of the driver used to resume virtual
   * machines. */
  public static final String METRIC_CONTAINER_RESUME_DRIVER = "container.driver.resume";

  /** Metric that indicates the identifier of the driver used to suspend virtual
   * machines. */
  public static final String METRIC_CONTAINER_SUSPEND_DRIVER = "container.driver.suspend";

  /** Metric that indicates the identifier of the driver used to run virtual
   * machines. */
  ...
  /** Metric that indicates the identifier of the driver to use to shutdown the
   * node. */
  ...
  private static boolean isSleeping(String str) {
  ...
  private static String extractVMFromSleeping(String str) {
  ...
  public void endDocument() throws SAXException {
    ...
    for (Node n : assigns.keySet()) {
      String vms = assigns.get(n);
      ...
      while (st.hasMoreElements()) {
        ...
        if (isSleeping(buf)) {
          ...
          if (vm == null) {
            // The configuration is incoherent and skipping the
            // virtual machine is not a viable solution.
            throw new SAXException("Unknown virtual machine '" + name + "'");
          } else {
            vm.setCPUConsumption(ConfigurationAdapter.getCPUConsumption(
                cpuPcts.get(vm), n));
            // We make the assignment
            currentConfiguration.setSleepOn(vm, n);
          }
        } else {
          String name = buf;
          VirtualMachine vm = allVMs.get(name);
          if (vm == null) {
            ...
            vm.setCPUConsumption(ConfigurationAdapter.getCPUConsumption(
                cpuPcts.get(vm), n));
            // We make the assignment
            currentConfiguration.setRunOn(vm, n);
          }
        }
      }
    }
  ...
  public void endElement(String uri, String localName, String name)
    ...
    if (name.equals("HOST")) {
      if (isContainer(currentMetrics.keySet())) {
        if (parent.getNodesBlackList().contains(currentHostname)) {
          Monitor.getLogger().debug(
              "Ignoring '" + currentHostname
                  + "': belong to the nodes black list");
        } else if (!parent.getNodesWhiteList().isEmpty()
            && !parent.getNodesWhiteList().contains(currentHostname)) {
          Monitor.getLogger().debug(
              "Ignoring '" + currentHostname
                  + "': do not belong to the nodes white list");
        } else if (onlines.contains(currentHostname)) {
          Monitor.getLogger().debug(
              currentHostname + " is considered as a container");
          float memKb = Integer.parseInt(currentMetrics
              .get(METRIC_CONTAINER_MEMORY_TOTAL));
          int cpuCapacity = ConfigurationAdapter.getCPUCapacity(
              Integer.parseInt(currentMetrics.get(METRIC_CPU_FREQUENCY)),
              Integer.parseInt(currentMetrics.get(METRIC_CONTAINER_NB_CPU)));
          Node n = new SimpleNode(currentHostname,
              Integer.parseInt(currentMetrics.get(METRIC_CONTAINER_NB_CPU)),
              cpuCapacity, (int) memKb / 1000); // Memory in MB

          // n.setHypervisorID(currentMetrics.get(METRIC_CONTAINER_TYPE));
          // n.setIPAddress(currentIp);
          // n.setStartupDriverID(currentMetrics
          // .get(METRIC_CONTAINER_STARTUP_DRIVER));
          // n.setShutdownDriverID(currentMetrics
          // .get(METRIC_CONTAINER_SHUTDOWN_DRIVER));
          // n.setMigrationDriverID(currentMetrics
          // .get(METRIC_CONTAINER_MIGRATION_DRIVER));
          // n.setRunDriverID(currentMetrics.get(METRIC_CONTAINER_RUN_DRIVER));
          // n.setResumeDriverID(currentMetrics
          // .get(METRIC_CONTAINER_RESUME_DRIVER));
          // n.setSuspendDriverID(currentMetrics
          // .get(METRIC_CONTAINER_SUSPEND_DRIVER));
          // n.setStopDriverID(currentMetrics.get(METRIC_CONTAINER_STOP_DRIVER));
          currentConfiguration.addOnline(n);
          assigns.put(n, currentMetrics.get(METRIC_CONTAINER_LIST_VMS));
        } else {
          float memKb = Integer.parseInt(currentMetrics
              .get(METRIC_CONTAINER_MEMORY_TOTAL));
          int cpuCapacity = ConfigurationAdapter.getCPUCapacity(
          ...
          // .get(METRIC_CONTAINER_SUSPEND_DRIVER));
          // n.setStopDriverID(currentMetrics.get(METRIC_CONTAINER_STOP_DRIVER));
          currentConfiguration.addOffline(n);
        }
      } else if (isVirtualMachine(currentMetrics.keySet())) {
        Monitor.getLogger().debug(
            currentHostname + " is considered as a virtual machine");
        // We just compute the pct of CPU consumption of the virtual
        // machine and store it
        // We compute the exact value during the assignment
        float cpuPct = Float
            .parseFloat(currentMetrics.get(METRIC_CPU_PCT_NICE))
            + Float.parseFloat(currentMetrics.get(METRIC_CPU_PCT_SYSTEM))
            + Float.parseFloat(currentMetrics.get(METRIC_CPU_PCT_USER));
        VirtualMachine vm = new SimpleVirtualMachine(currentHostname,
            Integer.parseInt(currentMetrics.get(METRIC_NB_CPU)), 0,
            Math.round(Float.parseFloat(currentMetrics
                .get(METRIC_VM_MEMORY_CONSUMPTION)) / 1000));
        cpuPcts.put(vm, cpuPct);
        allVMs.add(vm);
      } else {
        Monitor.getLogger().debug(currentHostname + " is ignored");
      }
    }/* else if (name.equals("HOST") && !this.isHostOnline) {
      * logger.debug(" Host '" + this.currentHostname +
  ...
  private static boolean isContainer(Set<String> metrics) {
  ...
  private static boolean isVirtualMachine(Set<String> metrics) {
  ...
  public void startDocument() throws SAXException {
    ...
    assigns = new HashMap<Node, String>();
    currentConfiguration = new SimpleConfiguration();
    cpuPcts = new HashMap<VirtualMachine, Float>();
  ...
  public void startElement(String uri, String localName, String name,
    ...
    if (name.equals("HOST")) {
      currentMetrics = new HashMap<String, String>();
      currentHostname = atts.getValue("NAME");
      atts.getValue("IP");
      int tn = Integer.parseInt(atts.getValue("TN"));
      int tmax = Integer.parseInt(atts.getValue("TMAX"));
      if (isMetricViable(tn, tmax)) {
        onlines.add(currentHostname);
      }
    } else if (name.equals("METRIC")) {
      currentMetrics.put(atts.getValue("NAME"), atts.getValue("VAL"));
    }
  }
  ...
  /** Indicates wheter a metric if viable or not, given a tolerance and the delay
  ...
  private static boolean isMetricViable(int tn, int tmax) {
    return tn < 4 * tmax;
  }
}
