...
public final class EntropyLauncher {
  ...
  public static void usage() {
    ...
    System.out
        .println("config_file: the path to the configuration file of entropy (default is '"
            + DEFAULT_PROPERTIES + "')");
  }

  /** Launcher of Entropy.
   * @param args first argument must be start or stop. Secong argument is
   * optional and denotes the configuration file of Entropy */
  public static void main(String[] args) {
    ...
    try {
      ...
      Entropy.getLogger().error(
          "Unable to read the configuration file: " + e.getMessage());
      System.exit(1);
    }
    ...
    if (args[0].equals("start")) {
      ...
      try {
        int port = properties
            .getRequiredPropertyAsInt(EntropyBuilder.REGISTRY_PORT_KEY);
        ...
        conn.invoke(new ObjectName(Entropy.MBEAN_NAME), "shutdown",
            new Object[] {}, new String[] {});
      }
      catch (IOException e) {
        Entropy.getLogger().error(
            "Unable to shutdown entropy: " + e.getMessage());
        System.exit(1);
      }
