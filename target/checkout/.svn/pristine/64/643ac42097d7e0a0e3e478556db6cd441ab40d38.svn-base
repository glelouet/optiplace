...
public final class DataConverter {
  ...
  public static void main(String[] args) {
    ...
    if (args.length < 5) {
      ...
      System.out
          .println("Usage: dataConv [-plan | -cfg | -vjob] -of output_format input_files output");
      System.out
          .println("input_files: series of files. Format will de  inferred from the files extension");
      ...
      System.out
          .println("Warning, as Plasma is untyped, a configuration must be given as a first input file to make"
  ...
  private static void convertConfiguration(List<String> inputs,
      String outputFormat, String outputPath) {
    FileConfigurationSerializer out = FileConfigurationSerializerFactory
        .getInstance().getSerializer(outputFormat);
    if (out == null) {
      fatal("Unsupported output format: " + outputFormat);
    }

    for (String input : inputs) {
      FileConfigurationSerializer src = FileConfigurationSerializerFactory
          .getInstance().getSerializer(input);
      if (src == null) {
        System.err.println("Skipping '" + input
            + "': not compatible with the input format");
        continue;
      }
  ...
  private static void convertPlan(List<String> inputs, String outputFormat,
      String outputPath) {
    FileTimedReconfigurationPlanSerializer out = FileTimedReconfigurationPlanSerializerFactory
        .getInstance().getSerializer(outputFormat);
    if (out == null) {
      fatal("Unsupported output format: " + outputFormat);
    }
    ...
    for (String input : inputs) {
      ...
      src = FileTimedReconfigurationPlanSerializerFactory.getInstance()
          .getSerializer(input);
      if (src == null) {
        System.err.println("Skipping '" + input
            + "': not compatible with the input format");
  ...
  private static void convertVJobs(List<String> inputs, String outputFormat,
      String outputPath) {
    VJobFileSerializerFactory out = DefaultVJobFileSerializerFactory
        .getInstance();
  ...
  private static String convertFileName(String path, String outputPath,
      String outputFormat) {
    File x = new File(path);
    String name = x.getName().substring(0, x.getName().lastIndexOf("."));
    return new StringBuilder(outputPath).append('/').append(name).append('.')
        .append(outputFormat).toString();
  }
}
