...
import static entropy.configuration.ResourcePicker.VMRc.*;
...
public class TestManagedElementSets {
	...
	public void testNodeSum() {
		...
		Assert.assertEquals(Arrays.toString(sum),
				Arrays.toString(new int[]{9, 18}));
	}
	...
	public void testNodeMin() {
		...
		Assert.assertEquals(minCpu.getCoreCapacity(), 1);
	...
	public void testNodeMax() {
		...
		Assert.assertEquals(maxCpu.getCoreCapacity(), 3);
	...
	public void testSum() {
		...
		int[] sum = ManagedElementSets.sum(s, cpuConsumption, cpuDemand,
				memoryConsumption, memoryDemand);
		Assert.assertEquals(Arrays.toString(sum),
				Arrays.toString(new int[]{9, 19, 16, 26}));
	}
	...
	public void testMin() {
		...
		VirtualMachine minMemCons = ManagedElementSets
				.min(s, memoryConsumption);
	...
	public void testMax() {
		...
		VirtualMachine maxMemCons = ManagedElementSets
				.max(s, memoryConsumption);
