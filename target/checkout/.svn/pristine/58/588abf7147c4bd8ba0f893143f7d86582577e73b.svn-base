...
public class ProtobufVJobBuilder implements VJobBuilder {
  ...
  // private static final Configuration emptyConfiguratin = new
  // SimpleConfiguration();
  ...
  public VJob build(File f) throws IOException, VJobBuilderException {
    try {
      ...
      for (PBVJob.vjob.VMDecl pbVm : pbV.getVmList()) {
        ...
        if (vm == null) {
          throw new VJobBuilderException("Unable to declare virtual machine '"
              + id);
        }
      ...
      for (PBVJob.vjob.Constraint pbc : pbV.getConstraintList()) {
        ...
        if (b == null) {
          throw new VJobBuilderException("Constraint '" + pbc.getId()
              + "' unknown");
        }
