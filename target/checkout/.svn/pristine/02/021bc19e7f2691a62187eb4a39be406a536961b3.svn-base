...
/** Adapter that extract a configuration from a Ganglia meta daemon. The adapter
 * create a connection to the ganglia meta daemon and retrieve an XML output of
 * the current monitoring metrics.
...
public class GangliaConfigurationAdapter extends ConfigurationAdapter {
  ...
  public String readXMLDump() throws MonitoringException {
    ...
    try {
      ...
      throw new MonitoringException("Unknown host: " + host + ":" + port, e);
    }
    catch (IOException e) {
      throw new MonitoringException(
          "Unable to get the monitoring report from the GMeta daemon", e);
    }
  ...
  public Configuration parseConfiguration(String buffer)
    ...
    try {
      ...
      throw new MonitoringException(
          "Error while parsing the XML stream of GMetad", e);
    }
    catch (IOException e) {
  ...
  public Configuration extractConfiguration() throws MonitoringException {
    return parseConfiguration(readXMLDump());
  }
