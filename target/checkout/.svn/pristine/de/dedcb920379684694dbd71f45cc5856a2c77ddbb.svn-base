...
/**
 * A heuristic to select the VM group variable to assign to a value. Biggest
 * groups (ie groups that require the maximum amount of resources) are selected
 * first.
 * 
...
public class VMGroupVarSelector extends AbstractIntVarSelector {
	...
	private static IntDomainVar[] makeVars(ReconfigurationProblem s) {
		...
		VirtualMachinesSetComparator cmp = new VirtualMachinesSetComparator(
				true, ResourcePicker.VMRc.cpuDemand);
		...
		int i = 0;
		for (ManagedElementSet<VirtualMachine> managedElementSet : groups) {
			vs[i++] = s.getVMGroup(managedElementSet);
		}
		return vs;
	}
