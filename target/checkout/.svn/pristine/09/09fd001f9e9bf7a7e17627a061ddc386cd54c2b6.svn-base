...
/** Simplify the configuration if the VMs are "CPU burning". If all the nodes
 * have the same CPU capacities, then it becomes equals to the number of
 * physical CPUs.
 * <p/>
 * Moreover, if the cpu consumption of a virtual machine is below a threshold,
 * the simplificaion makes it consumes a entire physical CPU. Otherwise, it
 * consumption is considered as neglictible and becomes equal to 0
...
public class CPUBurningVMsSampler implements ConfigurationSampler {
  ...
  public Configuration sample(Configuration c) {
    ...
    for (VirtualMachine vm : c.getRunnings()) {
      ...
      if (copy.getCPUConsumption() > threshold) {
        copy.setCPUConsumption(copy.getNbOfCPUs());
      } else {
        copy.setCPUConsumption(0);
      }
      newConf.setRunOn(copy,
          newConf.getOnlines().get(c.getLocation(vm).getName()));
    }
