...
public abstract class ComposedMultiSet<T extends ManagedElement> implements
VJobMultiSet<T> {
   ...
   * @return the description of the first set, then the operation, then the
   * second set */
  @Override
  public String toString() {
    ...
    if (first() instanceof ComposedMultiSet
        && !((ComposedMultiSet<?>) first()).operator().equals(operator())) {
      usePara = true;
    }
    ...
    usePara = false;
    if (second() instanceof ComposedMultiSet
        && !((ComposedMultiSet<?>) second()).operator().equals(operator())) {
      usePara = true;
    }
  ...
  public ManagedElementSet<T> getElements() {
    ManagedElementSet<T> all = new SimpleManagedElementSet<T>();
    Set<ManagedElementSet<T>> l = expand();
    for (ManagedElementSet<T> s : l) {
      all.addAll(s);
    }
    return all;
  }
