...
public class CapacityBuilder implements PlacementConstraintBuilder {
  ...
  @Override
  public Capacity buildConstraint(List<VJobElement> args)
      throws ConstraintBuilderException {
    if (args.size() != 2) {
      throw new ConstraintBuilderException(this);
    }
    try {
      ManagedElementSet<Node> ns = (ManagedElementSet<Node>) args.get(0);
      if (ns.isEmpty()) {
      ...
      int v = e.getValue();
      if (v < 0) {
        throw new ConstraintBuilderException("Negative capacity not allowed ("
            + v + ")");
      }
