...
import entropy.configuration.resources.ResourceUse;
import entropy.core.choco.reified.FastIFFNEQ;
import entropy.core.choco.reified.IntImplies;
import entropy.solver.PlanException;
import entropy.solver.SolutionStatistics;
import entropy.solver.SolvingStatistics;
...
public final class DefaultReconfigurationProblem extends CPSolver
	...
	private final ManagedElementSet<VirtualMachine> manageable;
	...
	private final ManagedElementSet<VirtualMachine> runnings;

	/** The future waiting VMs. */
	private final ManagedElementSet<VirtualMachine> waitings;

	/** The future sleeping VMs. */
	private final ManagedElementSet<VirtualMachine> sleepings;

	/** The future terminated VMs. */
	private final ManagedElementSet<VirtualMachine> terminated;
	...
	/**
	 * Build a reconfiguration problem. All the VMs are candidate for management
	 * 
	 * @param src
	 *            the source configuration
	 * @param run
	 *            the virtual machines to run
	 * @param wait
	 *            the virtual machines that will stay waiting
	 * @param sleep
	 *            the virtual machines to turn into the sleeping state
	 * @param stop
	 *            the virtual machines to stop
	 * @param on
	 *            the nodes to turn on
	 * @param off
	 *            the nodes to turn off
	 * @param eval
	...
	public DefaultReconfigurationProblem(Configuration src,
			...
			ManagedElementSet<VirtualMachine> stop) throws PlanException {
		this(src, run, wait, sleep, stop, src.getAllVirtualMachines());
	 ...
	 * @throws entropy.solver.PlanException
	...
	public DefaultReconfigurationProblem(Configuration src,
			...
			ManagedElementSet<VirtualMachine> manageable) throws PlanException {
		source = src;
	...
	private void makeConstantConfig() {
		...
		allVMs.addAll(waitings);
		allVMs.addAll(runnings);
	...
	public ManagedElementSet<Node> getFutureOnlines() {
		return getSourceConfiguration().getAllNodes();
	}

	@Override
	public ManagedElementSet<VirtualMachine> getManageable() {
		return manageable;
	}
	...
	protected void makeIsMigrateds() {
		...
		for (int i = 0; i < isMigrateds.length; i++) {
			VirtualMachine vm = getVirtualMachine(i);
			if ((getFutureRunnings().contains(vm) || getFutureSleepings()
					.contains(vm))
					&& (getSourceConfiguration().isRunning(vm) || getSourceConfiguration()
							.isSleeping(vm))) {
				isMigrateds[i] = isDifferent(host(vm),
						node(getSourceConfiguration().getLocation(vm)));
			} else {
				isMigrateds[i] = createIntegerConstant(0);
			}
		}
	...
	public Configuration extractConfiguration() {
		...
		for (VirtualMachine vm : getFutureSleepings()) {
			cfg.setSleepOn(vm, getNode(host(vm).getVal()));
		}
		return cfg;
	}
   ...
   */
	...
	/**
	 * force to have the same name for every constant
	 */
	@Override
	...
	public IntDomainVar createIntegerConstant(int val) {
		return this.createIntegerConstant(null, val);
	}
