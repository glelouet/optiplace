...
/** Analyse continuously the state of the infrastructure and reveals the number
 * of inconsistency
...
public class StateAnalyzer {
  ...
  public void analyze(Configuration cfg, List<VJob> vjobs,
      boolean isReconfiguring) {

    int nbVMs = cfg.getAllVirtualMachines().size();

    ManagedElementSet<VirtualMachine> in = new SimpleManagedElementSet<VirtualMachine>();
    ...
    ManagedElementSet<Node> overloaded = ConfigurationUtils
        .futureOverloadedNodes(cfg);
      ...
      for (PlacementConstraint c : vjob.getConstraints()) {
        if (!c.isSatisfied(cfg)) {
          if (c.getClass().equals(Ban.class)) {
            for (VirtualMachine vm : c.getMisPlaced(cfg)) {
              if (!in.contains(vm)) {
                vms.add(vm);
              }
            }
            ...
            DecisionModule.getLogger().info(
                c + " is not satisfied, but ignored by StateAnalyzer");
          }
        }
      }
    }
    ...
    try {
      store(nbVMs, nbWithContentions, nbMisPlaced, nbOfflines, isReconfiguring);
    }
    catch (IOException e) {
      DecisionModule.getLogger().error(
          "Unable to store analyse into '" + output + "'");
    }
  }

  private void store(int nbVMs, int nbWithContentions, int nbMisPlaced,
      int nbOfflines, boolean isReconfiguring) throws IOException {
    ...
    try {
      ...
      out.write(time + " " + nbVMs + " " + nbWithContentions + " "
          + nbMisPlaced + " " + nbOfflines + " "
          + (isReconfiguring ? "1" : "0") + "\n");
    }
    finally {
      if (out != null) {
