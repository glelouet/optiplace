...
public class ProtobufVJobBuilderBuilder implements VJobBuilderBuilder {
  ...
  @Override
  public ProtobufVJobBuilder build(VJobElementBuilder eb)
      throws VJobBuilderBuilderException {
    try {
      PropertiesHelper propHelper = new PropertiesHelper(file);

      PBConstraintsCatalog c = new ConstraintsCatalogBuilderFromProperties(
          propHelper).build();
      ...
      throw new VJobBuilderBuilderException(
          "Unable to build the protobuf vjob builder:" + e.getMessage(), e);
    }
