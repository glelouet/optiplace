...
/** A VJob scheduler based on the algorithm First Fit. The scheduler browse the
 * queue. For each vjob, it tests if a viable configuration exists. If it
 * suceeds at assigning all the VMs composing the vjob, then the VJobs will be
 * in the running state. Otherwise, depending on the current state of the job,
 * it will be in the waiting state or in the sleeping state. All the VMs that do
 * not belong to a VJobs will be stopped.
...
public class FirstFitVJobScheduler extends DecisionModule {
  ...
  @Override
  public Configuration compute(Configuration current)
    ...
    for (VJob l2 : declaredVJobs) {
      for (VirtualMachine vm : l2.getVirtualMachines()) {
        if (current.isRunning(vm)) {
          ...
          throw new AssignmentException("Unable to get the VM '" + vm.getName()
      ...
      for (VirtualMachine vm : toAssign) {
        for (Node node : tmpNodes) {
          if (vm.getCPUConsumption() <= node.getCPUCapacity()
            ...
            node.setMemoryCapacity(node.getMemoryCapacity()
      ...
      if (!success) {
        ...
        for (VirtualMachine vm : l2.getVirtualMachines()) {
          ...
          if (node != null) {
            tmpNodes.get(node.getName()).setCPUCapacity(
                node.getCPUCapacity() + vm.getCPUConsumption());
            tmpNodes.get(node.getName()).setMemoryCapacity(
                node.getMemoryCapacity() + vm.getMemoryConsumption());
          }
        }
      } else {
   ...
   * for (VirtualMachine vm : c.getRunnings()) { model.mustBeRunning(vm); } for
   * (VirtualMachine vm : c.getWaitings()) { model.mustBeOnFarm(vm); //Store
   * informations about their consumption this.suspendedInfos.remove(vm);
   * this.suspendedInfos.add(vm); } for (VirtualMachine vm : c.getSleepings()) {
   * model.mustBeOnFarm(vm); } model.lazyMaintainResourcesAccessibility();
   * VirtualMachineAssignmentSolver solver = new
   * VirtualMachineAssignmentSolver(); solver.read(model);
   * //solver.setVirtualMachineSelector(new
   * BiggestUnassignedVirtualMachine(solver, solver.getVar(nodes),
   * VirtualMachine.CPU_CONSUMPTION)); //solver.setNodeSelector(new
   * BusiestNode(solver, solver.getVar(nodes), Node.CPU_CAPACITY, true));
   * solver.setVirtualMachineSelector(new BiggestVirtualMachinesFirst(solver,
   * VirtualMachine.CPU_CONSUMPTION)); solver.setNodeSelector(new
   * StayFirstNodeSelector(solver)); if (solver.solve(this.getTimeout())) {
   * return solver.getResultingConfiguration(); } return
   * solver.getResultingConfiguration(); } */
}
