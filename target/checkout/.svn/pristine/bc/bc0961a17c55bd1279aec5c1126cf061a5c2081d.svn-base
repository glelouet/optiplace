		...
		for (Node n : nodes) {
			if (cfg.isOffline(n)) {
				...
				if (!cpy.containsAll(runs)) {
					throw new ConfigurationsException(cfg, "VMs on node "
							+ n.getName() + "(" + runs
				...
				throw new ConfigurationsException(cfg, "Unknown node "
						+ n.getName());
			}
		}
		if (!cfg.getWaitings().isEmpty() && !cfg.getWaitings().containsAll(cpy)) {
			throw new ConfigurationsException(cfg, "Waiting VMs ("
					+ cfg.getWaitings() + ") does not only contains VMs in "
					+ cpy);
		}
