...
/** A Client to execute actions on the virtual machines hosted on a Xen
 * Hypervisor. This client uses the xen-api 1.0.6 In order to execute methods,
 * you have to log in first using an username and a password. If the
 * authentification succeed, a session is opened and you can execute methods.
 * Then, you have to close your session using the method logout.
...
public class XenRpcClient {
  ...
  /** Make a new client and connect it to the port {@value #DEFAULT_XEN_API_PORT}
   * of a remote host.
  ...
  public boolean login(String username, String password)
      throws XenClientException, XmlRpcException {
    sessionID = (String) execute("session.login_with_password", new Object[] {
        username, password });
  ...
  public boolean logout() throws XenClientException, XmlRpcException {
    return execute("session.logout", new Object[] { sessionID }) != null;
  }

  /** Migrate a virtual machine on a destination node. The remote relocation port
   * must be {@value #DEFAULT_RELOCATION_PORT}
   ...
   * @throws XmlRpcException if an error occurred
   * @throws XenClientException if an error related to the operations occurred. */
  public boolean migrate(String name, String dest, boolean live)
      throws XenClientException, XmlRpcException {
   ...
   * @throws XmlRpcException if an error occurred
   * @throws XenClientException if an error related to the operations occurred. */
  public boolean migrate(String name, String dest, boolean live, int dstPort)
      throws XenClientException, XmlRpcException {
    Map<String, Object> m = new HashMap<String, Object>();
    m.put("port", dstPort);
    return execute("VM.migrate", new Object[] { sessionID, getUUID(name), dest,
        live, m }) != null;
  }

  /** Save the virtual machine in a state file and free associated CPU and memory
   * resources.
   * @param name the name of the virtual machine
   ...
   * @throws XmlRpcException if an error occurred
   * @throws XenClientException if an error related to the operations occurred. */
  public boolean save(String name, String stateFile, boolean checkpoint)
      throws XenClientException, XmlRpcException {
    String uuid = getUUID(name);
    if (uuid != null) {
      return execute("VM.save", new Object[] { sessionID, uuid, stateFile,
          checkpoint }) != null;
    }
    return false;
  }
  ...
  private Object execute(String method, Object[] params)
      throws XmlRpcException, XenClientException {
    HashMap<String, Object> result = (HashMap<String, Object>) client.execute(
        method, params);
    if (result.get("Status").equals("Success")) {
      ...
      if (toks[0].equals("PERMISSION_DENIED")) {
        ...
        throw new XenClientException(method, toks[0].toString(),
            "Incorrect number of parameters, expected " + toks[1].toString()
                + " but given " + toks[2].toString());
      } else if (toks[0].equals("SESSION_AUTHENTIFICATION_FAILED")) {
        throw new XenClientException(method, toks[0].toString(),
            "The given session ID '" + params[0] + "' is incorrect");
      } else if (toks[0].equals("SESSION_INVALID")) {
        throw new XenClientException(method, toks[0].toString(),
            "The given session ID '" + params[0]
                + "' is invalid (server restart or ID timed out)");
      } else if (toks[0].equals("VALUE_NOT_SUPPORTED")) {
        throw new XenClientException(method, toks[0].toString(), "The value '"
            + toks[2].toString() + " of the field '" + toks[1].toString()
            + " is not supported : " + toks[3].toString());
      } else if (toks[0].equals("VM_BAD_POWER_STATE")) {
        throw new XenClientException(
            method,
            toks[0].toString(),
            "The virtual machine '"
                + toks[1].toString()
                + "' is not in the good state to perform the operation. Expected '"
                + toks[2] + "' but was '" + toks[3] + "'");
      } else if (toks[0].equals("SECURITY_ERROR")) {
        throw new XenClientException(method, toks[0].toString(),
            "Security error'" + toks[1].toString() + " - " + toks[2].toString());
      } else {
        throw new XenClientException(method, toks[0].toString(), "");
      }
    }
   ...
   * @throws XmlRpcException if an error occurred
   * @throws XenClientException if an error related to the operations occurred. */
  public boolean restore(String stateFile, boolean run)
      throws XenClientException, XmlRpcException {
    return execute("VM.restore", new Object[] { sessionID, stateFile, run }) != null;
  }

  /** Get the UUID associated to the name of the virtual machine. The name must
   * be unique
   * @param name the name of the virtual machine
  ...
  public String getUUID(String name) throws XenClientException, XmlRpcException {
    Object[] res = (Object[]) execute("VM.get_by_name_label", new Object[] {
        sessionID, name });
    if (res.length > 0) {
   ...
   * @throws XmlRpcException if an error occurred
   * @throws XenClientException if an error related to the operations occurred. */
  public String getNameFromUUID(String uuid) throws XenClientException,
      XmlRpcException {
    return (String) execute("VM.get_name_label",
        new Object[] { sessionID, uuid });
  }
   ...
   * @throws XmlRpcException if an error occurred
   * @throws XenClientException if an error related to the operations occurred. */
  public boolean shutdown(String name) throws XenClientException,
      XmlRpcException {
    return execute("VM.clean_shutdown",
        new Object[] { sessionID, getUUID(name) }) != null;
  }
   ...
   * @return true if the operation succeed
   * @throws XenClientException if an error related to the operations occurred. */
  public boolean destroy(String name) throws XenClientException,
      XmlRpcException {
    return execute("VML.hard_shutdown",
        new Object[] { sessionID, getUUID(name) }) != null;
  }
  ...
  public List<String> listVMs() throws XenClientException, XmlRpcException {
    ...
    Object[] list = (Object[]) execute("VM.get_all", new Object[] { sessionID });
    for (Object o : list) {
      vms.add(getNameFromUUID(o.toString()));
    }
    return vms;
  }
