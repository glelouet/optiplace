...
public class TestMultiSetsUnion {
  ...
  public void testExpandAndAllElements() {
    ...
    Set<ExplodedSet<SimpleNode>> ns = new HashSet<ExplodedSet<SimpleNode>>();
    ExplodedSet<SimpleNode> all = new ExplodedSet<SimpleNode>();
    ExplodedSet<SimpleNode> s = new ExplodedSet<SimpleNode>();
    ExplodedSet<SimpleNode> e = new ExplodedSet<SimpleNode>();
    ExplodedMultiSet<SimpleNode> ms = ms1;
    ...
    for (int i = 1; i <= 20; i++) {
      ...
      if (i % 5 == 0) {
        ms.add(s);
        s = new ExplodedSet<SimpleNode>();
        ns.add(e);
        e = new ExplodedSet<SimpleNode>();
      }
      if (i == 10) {
        ms = ms2;
      }
    }
    MultiSetsUnion<SimpleNode> u = new MultiSetsUnion<SimpleNode>(ms1, ms2);
    Assert.assertEquals(u.expand(), ns);
    System.err.println(u.getElements());
    System.err.println(all);
    Assert.assertTrue(u.getElements().equals(all));
  }
