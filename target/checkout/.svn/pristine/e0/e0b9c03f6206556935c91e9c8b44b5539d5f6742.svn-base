...
public abstract class ConfigurationAdapter {
  ...
  public ConfigurationAdapter() {
    nodesBl = new LinkedList<String>();
    nodesWl = new LinkedList<String>();
  }
  ...
  /** Get the CPU consumption of a virtual machine. The virtual machine must have
   * 1 virtual CPU.
  ...
  public static int getCPUConsumption(float cpuPct, Node host) {
    return round(
        (int) (cpuPct * host.getCPUCapacity() / host.getNbOfCPUs() / 100),
        DEFAULT_CPU_STEP);
  }
