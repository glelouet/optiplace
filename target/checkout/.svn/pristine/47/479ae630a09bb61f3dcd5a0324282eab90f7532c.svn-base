...
public class TestSymbolsTable {
  ...
  public void testPrologUsage() {
    PlasmaVJob p = new BasicPlasmaVJob("prolog");
    ...
    ExplodedMultiSet<VirtualMachine> e4 = new ExplodedMultiSet<VirtualMachine>(
        "$multiVM");
    p.addNodes(e1);
    p.addNodes(e3);
    p.addVirtualMachines((VJobSet) e2);
  ...
  public void testDeclare() {
    ...
    es[0] = new ExplodedSet<SimpleNode>("$R1");
    cs[0] = new Content(Content.Type.nodeset, es[0]);

    es[1] = new ExplodedSet<SimpleVirtualMachine>("$T1");
    cs[1] = new Content(Content.Type.vmset, es[1]);

    es[2] = new ExplodedMultiSet<SimpleNode>("$small");
    cs[2] = new Content(Content.Type.multinodesets, es[2]);

    es[3] = new ExplodedMultiSet<SimpleVirtualMachine>("$multiVM");
  ...
  public void testBadDeclares() {
    ...
    s.declare("$R1", new Content(Content.Type.nodeset,
        new ExplodedSet<SimpleNode>("$R1")));
    // Label already exists
    Assert.assertFalse(s.declare("$R1", new Content(Content.Type.nodeset,
        new ExplodedSet<SimpleNode>("$R1"))));
  }
}
