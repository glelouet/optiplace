...
public class TestSSHRun {
  ...
  public void testGetCommandToExecute() {
    Run r = new Run(new SimpleVirtualMachine("myVirtualMachine", 1, 2, 3),
        new SimpleNode("theSource", 1, 2, 3));
    try {
      PropertiesHelper props = TestHelper.readEntropyProperties(RESOURCES_BASE
          + "testGets.txt");
      SSHRun s = new SSHRun(r, props);
      Assert
          .assertEquals(s.getCommandToExecute(),
              "plz run my VM. Its name iz myVirtualMachine. It will be fine in theSource");
    }
    catch (Exception e) {
  ...
  /** Driver used by default, so we check if initial parameters are good. */
  public void testDefaultProperties() {
    Run r = new Run(new SimpleVirtualMachine("vm1", 1, 2, 3), new SimpleNode(
        "n1", 1, 2, 3));
    try {
