...
public class TestEntropyLauncher {
  ...
  class ExitException extends SecurityException {
    ...
    public String getMessage() {
      return "Application execute a 'System.exit(" + status + ")'";
    }
  }

  /** A Mock security manager to "transform" a System.exit() into a
   * ExitException.
   * @author Fabien Hermenier */
  ...
  /* public void testRemoteShutdown() { System.setSecurityManager(new
   * NoExitSecurityManager()); try { final MockControlLoop mock = new
   * MockControlLoop(); Entropy e = new Entropy(mock); e.setRegistryPort(4500);
   * e.setSleepDelay(1); e.startup(); EntropyLauncher.main(new String []
   * {"stop",
   * "src/test/resources/entropy/TestEntropyLauncher.entropy_properties.txt" });
   * } catch (Exception e) { Assert.fail(e.getMessage(), e); } finally {
   * System.setSecurityManager(null); } } */
