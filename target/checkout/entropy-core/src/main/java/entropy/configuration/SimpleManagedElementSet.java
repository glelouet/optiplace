...
/**
 * Basic implementation of a ManagedElementSet. {@code add()},{@code addAll()},
 * {@code contains()}, , {@code get()}, , {@code set()} operations are performed
 * in an almost constant time. {@code remove()}, {@code removeAll()} operations
 * are in a constant time if the element is not in the set. O(n) otherwise
 * 
 * @author Fabien Hermenier
 */
public class SimpleManagedElementSet<E extends ManagedElement>
		extends
			ArrayList<E> implements ManagedElementSet<E>, Cloneable {
   ...
   */
	...
	public boolean remove(E o) {
		return map.remove(o.hashCode()) != null && super.remove(o);
	}
	...
	public E set(int i, E e) {
		E old = super.set(i, e);
		int h = e.hashCode();
		// e already in, so it is just a position change, no need to update the
		// map
		if (this.map.contains(h)) {
	...
	public String toString() {
		...
		for (Iterator<E> ite = iterator(); ite.hasNext();) {
			E e = ite.next();
			b.append(e.getName());
			if (ite.hasNext()) {
				b.append(", ");
