...
/**
 * Serialize and un-serialize a configuration from/to a plain text format, human
 * readable. Mostly the original format of a configuration.
 * 
 * @author Fabien Hermenier
 */
public final class PlainTextConfigurationSerializer
		extends
			FileConfigurationSerializer {

	private static final Logger logger = LoggerFactory
			.getLogger(PlainTextConfigurationSerializer.class);
	...
	/** Index of the CPU max consumption value. */
	public static final int VM_MAX_CPU = 4;
	...
	public static Configuration unSerialize(BufferedReader reader)
			throws IOException, ConfigurationSerializerException {
		...
		while (line != null && !line.equals(END_CONFIG)
				&& !line.startsWith(START_PLAN)) {
			if (line.startsWith(LIST_NODES)) {
				...
				if (state == null) {
					throw new ConfigurationSerializerException(
							"Unrecognized characters: " + line);
					...
					if (toks[0].equals(FARM)) {
						...
						if (n != null) {
							...
							throw new ConfigurationSerializerException("Node '"
									+ id + "' is unknown");
						}
					} else {
	...
	@Override
	public Configuration unSerialize(InputStream in) throws IOException,
			ConfigurationSerializerException {
		return unSerialize(new BufferedReader(new InputStreamReader(in)));
	}

	public static Configuration read(String folder, String filename) {
		File file = new File(folder, filename);
		if (!file.exists() || !file.isFile()) {
			File dir = new File(folder);
			if (!dir.exists() || !dir.isDirectory()) {
				logger.warn("cannot load config from dir " + folder);
			} else {
				logger.warn("cannot find file " + filename + " in " + folder);
			}
			return null;
		}
		try {
			return unSerialize(new BufferedReader(new FileReader(file)));
		} catch (Exception e) {
			logger.warn("while unserializing file " + filename + " in dir "
					+ folder, e);
			return null;
		}
	}
	...
	private static String writeVirtualMachine(VirtualMachine vm) {
	...
	private static VirtualMachine readVirtualMachine(String line)
			throws ConfigurationSerializerException {
		String[] toks = line.split(FIELD_SEP);
		if (toks.length < VM_NB_MIN_TOKENS) {
			throw new ConfigurationSerializerException("'" + line
					+ "' should have at lease " + VM_NB_MIN_TOKENS + " tokens");
		}
		...
		if (toks[VM_CONSO_CPU_IDX].contains(VM_DEMAND_SEP)) {
			...
			consoCPU = Integer.parseInt(toks[VM_CONSO_CPU_IDX].substring(0,
					toks[VM_CONSO_CPU_IDX].indexOf(VM_DEMAND_SEP)));
			needCPU = Integer.parseInt(toks[VM_CONSO_CPU_IDX].substring(
					toks[VM_CONSO_CPU_IDX].indexOf(VM_DEMAND_SEP) + 2,
					toks[VM_CONSO_CPU_IDX].length()));
			// System.err.println(toks[VM_CONSO_CPU_IDX] + " " + "|" + consoCPU
			// + "| |" + needCPU+"|");
			/*
			 * if ((ss.length != 2) && (ss[0].length() == 0 || ss[1].length() ==
			 * 0)) { throw new
			 * ConfigurationSerializerException("Bad syntax for '" +
			 * toks[VM_CONSO_CPU_IDX] + "'. '" + VM_DEMAND_SEP +
			 * "' must be between 2 integers, whitout spaces"); }
			 */
		...
		if (toks[VM_CONSO_MEM_IDX].contains(VM_DEMAND_SEP)) {
			...
			if (ss.length != 2 && (ss[0].length() == 0 || ss[1].length() == 0)) {
				throw new ConfigurationSerializerException("Bad syntax for '"
						+ toks[VM_CONSO_MEM_IDX] + "'. '" + VM_DEMAND_SEP
						+ "' must be between 2 integers, whitout spaces");
			}
		...
		int maxCPU = -1;
		if (toks.length >= VM_MAX_CPU + 1) {
			maxCPU = Integer.parseInt(toks[VM_MAX_CPU]);
		}

		if (nbCPU <= 0 || consoCPU < 0 || consoMem < 0) {
			throw new ConfigurationSerializerException("Incorrect value for '"
					+ line + "'. all numbers must be positive");
		}
		VirtualMachine vm = new SimpleVirtualMachine(name, nbCPU, consoCPU,
				consoMem);
		if (needCPU >= 0) {
		...
		vm.setCPUMax(maxCPU);
		/*
		 * if (toks.length == 5) { vm.updateValue(DefaultVirtualMachine.VJOB_ID,
		 * toks[VM_LEASE_IDX]); }
		 */
		return vm;
	}
	...
	private static Node readNode(String line)
		...
		try {
			...
			if (nbCPU <= 0 || capaCPU < 0 || capaMem < 0) {
				throw new ConfigurationSerializerException(
						"Incorrect value for '" + line
								+ "'. all numbers must be positive");
			}
			return new SimpleNode(name, nbCPU, capaCPU, capaMem);
		} catch (NumberFormatException e) {
			throw new ConfigurationSerializerException(
					"Unable to create a Node from '" + line + "':"
							+ e.getMessage());
		}
	}

	private static String writeNode(Node node) {
		StringBuilder buffer = new StringBuilder(100);
		buffer.append(node.getName());
		buffer.append(" ");
		buffer.append(node.getNbOfCores());
		buffer.append(" ");
		buffer.append(node.getCoreCapacity());
		buffer.append(" ");
