...
/**
 * An exception to describe a non viable source configuration for a planning
 * problem. A configuration is not viable if for one node, the sum of CPU
 * consumptions or memory consumptions is superior to the CPU or the memory
...
public class NonViableSourceConfigurationException extends PlanException {
	...
	/**
	 * Make a new exception related to a specific configuration and a sample
	 * node.
	 * 
	 * @param src
	 *            the source configuration
	...
	public NonViableSourceConfigurationException(Configuration src, Node n) {
		super("Source configuration is not viable: node '" + n.getName()
				+ "' is overloaded");
		source = src;
		node = n;
	}
