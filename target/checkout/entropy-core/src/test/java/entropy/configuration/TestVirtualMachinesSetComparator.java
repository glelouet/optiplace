...
public class TestVirtualMachinesSetComparator {
	...
	public void test1() {
		VirtualMachine vm1 = new SimpleVirtualMachine("VM1", 1, 1, 4);
		VirtualMachine vm2 = new SimpleVirtualMachine("VM2", 1, 2, 3);
		VirtualMachine vm3 = new SimpleVirtualMachine("VM3", 1, 3, 2);
		VirtualMachine vm4 = new SimpleVirtualMachine("VM4", 1, 4, 1);
		...
		VirtualMachinesSetComparator cmp = new VirtualMachinesSetComparator(
				true, ResourcePicker.VMRc.cpuConsumption);

		Assert.assertEquals(0, cmp.compare(s1, s1));
		Assert.assertEquals(-1, cmp.compare(s1, s2));
		Assert.assertEquals(1, cmp.compare(s2, s1));

		cmp = new VirtualMachinesSetComparator(false,
				ResourcePicker.VMRc.memoryConsumption);
		Assert.assertEquals(0, cmp.compare(s1, s1));
		Assert.assertEquals(-1, cmp.compare(s1, s2));
		Assert.assertEquals(1, cmp.compare(s2, s1));
	}
