...
public class TestSimpleConfiguration {
	...
	/**
	 * Make a default configuration for test purpose. VM0 to VM9 are affected to
	...
	public void testCopyConstructor() {
		...
		// Basic check
		Assert.assertEquals((Object) copy.getRunnings(),
				(Object) c.getRunnings());
		Assert.assertEquals((Object) copy.getWaitings(),
				(Object) c.getWaitings());
		Assert.assertEquals((Object) copy.getSleepings(),
				(Object) c.getSleepings());

		Assert.assertEquals((Object) copy.getOnlines(), (Object) c.getOnlines());
		Assert.assertEquals((Object) copy.getOfflines(),
				(Object) c.getOfflines());

		for (Node n : c.getOnlines()) {
			Assert.assertEquals((Object) c.getRunnings(n),
					(Object) copy.getRunnings(n));
			Assert.assertEquals((Object) c.getSleepings(n),
					(Object) copy.getSleepings(n));
		}
	...
	public void testGetLocation() {
		Configuration c = makeDefaultConfiguration();
		Assert.assertEquals(c.getLocation(new SimpleVirtualMachine("VM0", 1, 2,
				3)), c.getOnlines().get("N0"));
		Assert.assertNull(c
				.getLocation(new SimpleVirtualMachine("zob", 1, 2, 3)));

		Assert.assertEquals(c.getLocation(new SimpleVirtualMachine("VM15", 1,
				2, 3)), c.getOnlines().get("N5"));
		Assert.assertNull(c
				.getLocation(new SimpleVirtualMachine("zob", 1, 2, 3)));
	...
	/**
	 * Random modifications on a cloned configuration. The original should not
	 * be modified
	 */
	public void testClone() {
		...
		for (int i = 0; i < c2.getRunnings().size(); i++) {
			...
			// System.out.println(vm.getName() + " from " +
			// c2.getLocation(vm).getName() + " to " + n.getName());
			c2.setRunOn(vm, n);
