...
import entropy.core.packers.FastBinPacking;
...
public class TestFastBinPacking {
	...
	public void modelPack(int[] capa, int[] height) {
		...
		SConstraint<?> cPack = new FastBinPacking(s.getEnvironment(), loads,
				sizes, bins);
		s.post(cPack);
	}
	...
	public void testPack(int nbSol) {
		...
		if (nbSol > 0) {
			Assert.assertEquals(s.getNbSolutions(), nbSol,
					"#SOL=" + s.getNbSolutions());
		}
		s.clear();
	}
