...
public class StickVMsHeuristicTest {
	...
	public void testVMsByCPUTIMESMemIncreasing() {
		SimpleVirtualMachine vm0 = new SimpleVirtualMachine("vm0", 1, 10, 10);
		SimpleVirtualMachine vm1 = new SimpleVirtualMachine("vm1", 1, 20, 10);
		SimpleVirtualMachine vm2 = new SimpleVirtualMachine("vm2", 1, 11, 20);
		SimpleVirtualMachine vm3 = new SimpleVirtualMachine("vm3", 1, 20, 20);
		List<SimpleVirtualMachine> sortedOrder = Arrays.asList(vm0, vm1, vm2,
				vm3);
		...
		Collections.sort(unsorted,
				StickVMsHeuristic.VMS_BY_CPUTIMESMEM_INCREASING);
		Assert.assertEquals(sortedOrder, unsorted);
	}
	...
	public void testSimpleConfigurationRemaining() {
		...
		sp.getStrat().setGoal(new MigrationReducerGoal());
		sp.solve();
