...
/** Build VJobs from textual descriptions. A prolog vjob may be used to consider
...
public class PlasmaVJobBuilder implements VJobBuilder {
   ...
   * @throws entropy.vjob.builder.VJobBuilderException if an error occurred
   * while building the vjob
   * @throws IOException if an error occurred while reading the file */
  public PlasmaVJob build(String id, File f) throws VJobBuilderException,
      IOException {
    try {
  ...
  public PlasmaVJob build(String id, String description)
  ...
  private PlasmaVJob build(String id, CharStream cs)
      throws VJobBuilderException {
    ...
    parser.setTreeAdaptor(new VJobTreeAdaptor(errs, new SymbolsTable(prolog),
        elemBuilder, v, catalog));
    try {
   ...
   * @return a catalog, may be null if no catalog was specified as
   * instantiation. */
