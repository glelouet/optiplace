...
public class EqualsTree extends VJobTree {
  ...
  public EqualsTree(Token t, SemanticErrors errs, SymbolsTable syms,
      PlasmaVJob v) {
    super(t, errs);
    symbols = syms;
    vjob = v;
  ...
  public Content go(VJobTree parent) {
    if (symbols.isDeclared(getChild(0).getText())) {
      errors.append(((VJobTree) getChild(0)).token, "Variable "
          + getChild(0).getText() + " is already defined");
    } else {
      // We declare the variable even if the right operand has to be
      // ignored
