...
public class ConstraintBuilderException extends Exception {
  ...
  /** Make a new exception that signal a signature error when trying to build a
   * constraint
  ...
  public ConstraintBuilderException(PlacementConstraintBuilder builder) {
    super("Unable to build constraint '" + builder.getIdentifier()
        + "'. Expected signature: " + builder.getSignature());
  }
