...
public class GangliaAdapterBuilder implements ConfigurationAdapterBuilder {
   ...
   * @throws MonitorFactoryException if an error occurred while create the
   * adapter */
  @Override
  public GangliaConfigurationAdapter build(String props)
      throws MonitorFactoryException {
    try {
      PropertiesHelper helper = new PropertiesHelper(props);
      return new GangliaConfigurationAdapter(
          helper.getRequiredProperty(HOSTNAME_PROPERTY),
          helper.getRequiredPropertyAsInt(PORT_PROPERTY));
    }
    catch (MissingRequiredPropertyException e) {
      throw new MonitorFactoryException(
          "Unable to instantiate Gangia monitoring module: " + e.getMessage(),
          e);
    }
    catch (WrongPropertyTypeException e) {
      throw new MonitorFactoryException(
          "Unable to instantiate Ganglia monitoring module: " + e.getMessage(),
          e);
    }
    catch (IOException e) {
      throw new MonitorFactoryException(
