...
public class ExplodedIntervalTree extends VJobTree {
  ...
  public ExplodedIntervalTree(Token t, SemanticErrors errs,
      VJobElementBuilder eb) {
    super(t, errs);
    elemBuilder = eb;
  }
  ...
  public Content go(VJobTree parent) {
    ...
    String head = getChild(0).getText().substring(0,
        getChild(0).getText().length() - 1);
    ...
    range.append("]");
    range.append(tail);
    VirtualMachine vm = elemBuilder.matchVirtualMachine(first);
    if (vm != null) {
      ex = new RangeOfElements<VirtualMachine>(range.toString(),
          range.toString());
      for (int i = 1; i <= getChildCount() - 2; i++) {
        vm = elemBuilder.matchVirtualMachine(head + getChild(i).getText()
            + tail);
        if (vm == null) {
          errors.append(firstTok, "element " + head + getChild(i).getText()
              + tail + " is not a virtual machine");
