...
/** Driver to boot a node using Wake-On-Lan. As this method is not sure, the
 * magic packet can be sent multiple times.
...
public class WoLStartup extends Driver {
   ...
   * @param mac the mac in a String format: 6 hexadecimal number each separated
   * by "-" or ":"
  ...
  public void execute() throws DriverException {
    ...
    try {
      InetAddress ip = InetAddress.getByName(st.getNode().getName());
      String mac = st.getNode().getMACAddress();
      if (mac == null) {
        throw new DriverException(this,
            "Unable to get the MAC Address of the node '"
                + st.getNode().getName() + "'");
      }
