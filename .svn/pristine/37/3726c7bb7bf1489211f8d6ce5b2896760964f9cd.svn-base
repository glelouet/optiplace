...
@Test(groups = { "unit" })
public class TestFastANTLRDurationEvaluator {
  ...
  private DurationEvaluator makeDurationEvaluator() {
    try {
      PropertiesHelper prop = TestHelper.readEntropyProperties(RESOURCES_BASE
          + "expressions.properties");
      return FastDurationEvaluatorFactory.readFromProperties(prop);
    }
    catch (Exception e) {
  ...
  /** Test evaluateForge(). */
  public void testEvaluateForge() {
    DurationEvaluator ev = makeDurationEvaluator();
    VirtualMachine vm = new SimpleVirtualMachine("VM1", 1, 2, 3);
    try {
      Assert.assertEquals(ev.evaluateForge(vm), 5);
    }
    catch (DurationEvaluationException e) {
      Assert.fail(e.getMessage());
    }
  ...
  public void testEvaluateStartup() {
    DurationEvaluator ev = makeDurationEvaluator();
    Node n = new SimpleNode("N1", 1, 1, 1500);
    try {
  ...
  public void testEvaluateShutdown() {
    DurationEvaluator ev = makeDurationEvaluator();
    Node n = new SimpleNode("N1", 2, 1, 1);
    try {
