...
public class TestSpread {
  ...
  public void testMisPlaced() {
    ...
    vms.add(vm3);
    vms.add(vm4);
    Assert.assertTrue(new MockSpread(
        new SimpleManagedElementSet<VirtualMachine>()).getMisPlaced(cfg)
        .isEmpty());
    ManagedElementSet<VirtualMachine> s = new MockSpread(
        cfg.getAllVirtualMachines()).getMisPlaced(cfg);
  ...
  public void testIsSatisfied() {
    ...
    vms.add(vm4);
    Assert.assertTrue(new MockSpread(
        new SimpleManagedElementSet<VirtualMachine>()).isSatisfied(cfg));
    Assert.assertFalse(new MockSpread(cfg.getAllVirtualMachines())
        .isSatisfied(cfg));
