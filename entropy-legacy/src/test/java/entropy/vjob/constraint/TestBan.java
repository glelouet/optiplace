...
public class TestBan {
  ...
  public void testWithAllRunnings() {
    ...
    try {
      ...
      v.addConstraint(new Ban(new SimpleManagedElementSet<VirtualMachine>(vm4),
          new SimpleManagedElementSet<Node>(n3)));
      v.addConstraint(new Ban(new SimpleManagedElementSet<VirtualMachine>(vm4),
          new SimpleManagedElementSet<Node>(src.getAllNodes().get("N2"))));
      List<VJob> queue = new LinkedList<VJob>();
      queue.add(v);
      entropy.plan.ReconfigurationResult p = plan.compute(src, dst.getRunnings(),
          dst.getWaitings(), dst.getSleepings(),
          new SimpleManagedElementSet<VirtualMachine>(), dst.getOnlines(),
          dst.getOfflines(), queue);
  ...
  /** Test with a set composed with runnings & non-running VMs. The constraint is
   * not considered for non-running VMs. */
  public void testWithNonRunnings() {
    ...
    try {
      ...
      v.addConstraint(b1);
      // An entailed constraint, vm4 is sleeping so stay on a node
      // supposed to be avoided
      Ban b2 = new Ban(new SimpleManagedElementSet<VirtualMachine>(vm4),
          new SimpleManagedElementSet<Node>(n3));
      v.addConstraint(b2);
      List<VJob> queue = new LinkedList<VJob>();
      queue.add(v);
  ...
  public void testIsSatisfied() {
    ...
    Assert.assertTrue(new Ban(cfg.getAllVirtualMachines(),
        new SimpleManagedElementSet<Node>(n2)).isSatisfied(cfg));
    Assert.assertFalse(new Ban(cfg.getAllVirtualMachines(),
        new SimpleManagedElementSet<Node>(n1)).isSatisfied(cfg));
    Assert.assertFalse(new Ban(cfg.getAllVirtualMachines(), cfg.getAllNodes())
        .isSatisfied(cfg));
  }
