...
public class TestDefaultConstraintsCatalog {
  ...
  public void testBuild() {
    ...
    ExplodedMultiSet<SimpleVirtualMachine> e = new ExplodedMultiSet<SimpleVirtualMachine>();
  ...
  public void testBuildUnknown() {
    DefaultConstraintsCatalog c = new DefaultConstraintsCatalog();
    try {
      PlacementConstraint p = c.buildConstraint("toto",
          new LinkedList<VJobElement>());
      Assert.assertNull(p);
    }
    catch (ConstraintBuilderException ex) {
      Assert.fail(ex.getMessage());
