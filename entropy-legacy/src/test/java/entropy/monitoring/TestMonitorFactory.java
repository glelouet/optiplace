...
public class TestMonitorFactory {
  ...
  public void testMockCreation() {
    try {
      MonitorFactory f = new MonitorFactory(
          TestHelper
              .readEntropyProperties(RESOURCES_DIR + "createMockImpl.txt"));
      Monitor m = f.createMonitor();
      Assert
          .assertTrue(m.getConfigurationAdapter() instanceof MockConfigurationAdapter);
    }
    catch (Exception e) {
  ...
  public void testBadImplCreation() throws MonitorFactoryException {
    try {
      MonitorFactory f = new MonitorFactory(
          TestHelper.readEntropyProperties(RESOURCES_DIR + "createBadImpl.txt"));
      f.createMonitor();
    }
  ...
  @Test(dependsOnMethods = { "testLoadingMemorySampler",
      "testLoadingCPUSampler" })
  public void testDefaultProperties() {
    try {
      ...
      Monitor m = f.createMonitor();
      Assert
          .assertTrue(m.getConfigurationAdapter() instanceof GangliaConfigurationAdapter);

      // TODO: Check the default samplers
      Assert.assertEquals(m.getAttachedSamplers().size(), 0);
      /* Assert.assertTrue(m.getAttachedSamplers().get(0) instanceof
       * MemorySampler); Assert.assertEquals(((MemorySampler)
       * m.getAttachedSamplers().get(0)).getDivider(), 1024);
       * Assert.assertTrue(m.getAttachedSamplers().get(1) instanceof
       * CPUSampler); Assert.assertEquals(((CPUSampler)
       * m.getAttachedSamplers().get(1)).getDivider(), 250); */
  ...
  public void testBlackListLoading() {
    MonitorFactory f = new MonitorFactory(
        TestHelper.readEntropyProperties(RESOURCES_DIR + "testBLloading.txt"));
    try {
      Monitor m = f.createMonitor();
      Assert.assertEquals(m.getConfigurationAdapter().getNodesBlackList()
          .size(), 3);
      Assert.assertTrue(m.getConfigurationAdapter().getNodesBlackList()
          .contains("node1"));
      Assert.assertTrue(m.getConfigurationAdapter().getNodesBlackList()
          .contains("node5"));
      Assert.assertTrue(m.getConfigurationAdapter().getNodesBlackList()
  ...
  public void testBlackListLoadingWithBadFile() throws MonitorFactoryException {
    MonitorFactory f = new MonitorFactory(
        TestHelper
            .readEntropyProperties(RESOURCES_DIR + "testBadBLloading.txt"));
    try {
      f.createMonitor();
    }
    catch (PropertiesHelperException e) {
  ...
  public void testWhiteListLoading() {
    MonitorFactory f = new MonitorFactory(
        TestHelper.readEntropyProperties(RESOURCES_DIR + "testWLloading.txt"));
    try {
      Monitor m = f.createMonitor();
      Assert.assertEquals(m.getConfigurationAdapter().getNodesWhiteList()
          .size(), 3);
      Assert.assertTrue(m.getConfigurationAdapter().getNodesWhiteList()
          .contains("node1"));
      Assert.assertTrue(m.getConfigurationAdapter().getNodesWhiteList()
          .contains("node5"));
      Assert.assertTrue(m.getConfigurationAdapter().getNodesWhiteList()
  ...
  /** Test the detection of an error when both a black and a white lists are
   * defined.
   * @throws MonitorFactoryException The exception we expect */
  ...
  public void testAvoidingBlackAndWhiteList() throws MonitorFactoryException {
    MonitorFactory m = new MonitorFactory(
        TestHelper.readEntropyProperties(RESOURCES_DIR + "bothBLandWL.txt"));
    try {
      m.createMonitor();
    }
    catch (PropertiesHelperException e) {
  ...
  public void testLoadingMemorySampler() {
    MonitorFactory m = new MonitorFactory(
        TestHelper.readEntropyProperties(RESOURCES_DIR
            + "testMemorySampler.txt"));
    try {
  ...
  public void testLoadingCPUSampler() {
    MonitorFactory m = new MonitorFactory(
        TestHelper.readEntropyProperties(RESOURCES_DIR + "testCPUSampler.txt"));
    try {
      Monitor mon = m.createMonitor();
      Assert.assertEquals(mon.getAttachedSamplers().size(), 1);
      ConfigurationSampler s = mon.getAttachedSamplers().get(0);
