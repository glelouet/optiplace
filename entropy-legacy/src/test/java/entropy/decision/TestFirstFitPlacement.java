...
public class TestFirstFitPlacement {
  ...
  public void testStandard() {
    try {
      ...
      Configuration dst = p.compute(TestHelper.readConfiguration(RESOURCES_DIR
          + "testStandard.txt"));
      /* for (VirtualMachine vm : dst.getRunnings()) { System.out.println(vm); }
       * for (Node n : dst.getOnlines()) { System.out.println(n); }
       * System.out.println(dst); */
      Assert.assertEquals(ConfigurationUtils.currentlyOverloadedNodes(dst)
          .size(), 0);
      Assert.assertEquals(
          ConfigurationUtils.usedNodes(dst,
              EnumSet.of(ConfigurationUtils.State.Runnings)).size(), 3);
    }
    catch (Exception e) {
  ...
  public void testFirstFitDecrease() {
    try {
      VirtualMachineComparator cmp = new VirtualMachineComparator(false,
          ResourcePicker.VMRc.cpuConsumption);
      ...
      Assert.assertEquals(
          ConfigurationUtils.usedNodes(dst,
              EnumSet.of(ConfigurationUtils.State.Runnings)).size(), 2);
    }
    catch (Exception e) {
      Assert.fail(e.getMessage(), e);
    }
  ...
  public void testWithNoSolution() throws AssignmentException {
    Configuration src = TestHelper.readConfiguration(RESOURCES_DIR
        + "testWithNoSolution.txt");
    src.setRunOn(new SimpleVirtualMachine("VM5", 1, 0, 3000), src.getOnlines()
        .get(0));
    FirstFitPlacement p = new FirstFitPlacement();
    p.compute(src);
  }
