...
/** Unit tests for entropy.plan.ReconfigurationResultSolver.
 * @author Fabien Hermenier */
@Test(groups = { "unit", "RP-core" })
public class Testentropy.plan.ReconfigurationResultSolver {

  /** Location of resources used for tests. */
  public static final String RESOURCES_LOCATION = "src/test/resources/entropy/plan/choco/Testentropy.plan.ReconfigurationResultSolver.";

  /** Make a migration on a node previously offline. The node must be booted
  ...
  public void testMigrateOnaBootingNode() {
    ...
    entropy.plan.ReconfigurationResult plan = model.extractSolution();
  ...
  /** Test a plan where the destination of a migration has to be booted first
   * while the host is turned off after the migration. */
  public void testShutdownAnHostingNode() {
    ...
    ReconfigurationProblem model = TimedReconfigurationPlanModelHelper
        .makeBasicModel(src, dst);
    /* try { model.getEnd().setSup(30); } catch (ContradictionException e) {
  ...
  public void testBootingAnHostingNode() {
    ...
    entropy.plan.ReconfigurationResult plan = model.extractSolution();
    Configuration c = plan.getDestination();
    Assert.assertEquals(ConfigurationUtils.futureOverloadedNodes(c).size(), 0);
    // Check the plan
    Assert.assertEquals(plan.size(), 2);
    for (Action a : plan) {
  ...
  public void testIdlePlan() {
    ...
    entropy.plan.ReconfigurationResult plan = model.extractSolution();
    Configuration c = plan.getDestination();
    Assert.assertEquals(ConfigurationUtils.currentlyOverloadedNodes(c).size(),
        0);
  }
