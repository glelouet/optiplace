...
/** Unit tests for Basicentropy.plan.ReconfigurationResultModel. TODO: tests duration
 * bounds
...
public class TestDefaultReconfigurationProblem {
  ...
  public static final String RESOURCES_LOCATION = "src/test/resources/entropy/plan/choco/Testentropy.plan.ReconfigurationResultModel.";

  /** Just check the instantiation, access to the source configuration and start
   * and end moment. */
  public void testInstantiation() {
    Configuration src = TestHelper.readConfiguration(RESOURCES_LOCATION
        + "sample_src.txt");
    Configuration dst = TestHelper.readConfiguration(RESOURCES_LOCATION
        + "sample_src.txt");
    ...
    for (VirtualMachine vm : src.getRunnings()) {
      int vmIdx = model.getVirtualMachine(vm);
      int nIdx = model.getCurrentLocation(vmIdx);
      Node n = model.getNode(nIdx);
      Assert.assertEquals(src.getLocation(vm), n);
    }
  }

  /** Test the creation and the cache of group of VMs in various conditions. */
  /* public void testGetGroupOfVMs() { Configuration src =
   * TestHelper.readConfiguration(RESOURCES_LOCATION + "sample_src.txt");
   * Configuration dst = TestHelper.readConfiguration(RESOURCES_LOCATION +
   * "sample_src.txt"); ReconfigurationProblem model =
   * TimedReconfigurationPlanModelHelper.makeBasicModel(src, dst);
   ...
   * without overlapping ManagedElementSet<VirtualMachine> t2 = new
   * SimpleManagedElementSet<VirtualMachine>();
   ...
   * Assert.assertNotSame(v1, v2); //Last, a group with a VM already associated
   * to a group ManagedElementSet<VirtualMachine> t3 = new
   * SimpleManagedElementSet<VirtualMachine>();
   * t3.add(src.getRunnings().get("VM5")); t3.add(src.getRunnings().get("VM6"));
   * t3.add(src.getRunnings().get("VM4"));
   * Assert.assertNull(model.getVMGroup(t3)); //No change
   * Assert.assertEquals(model.getVMGroups().size(), 2); Assert.assertNull(model
   * .getAssociatedGroup(src.getRunnings().get("VM5"))); Assert.assertNull(model
   * .getAssociatedGroup(src.getRunnings().get("VM6"))); Assert.assertEquals(v2,
   * model.getAssociatedGroup(src.getRunnings().get("VM4"))); } */

  /** Test the creation and the cache of group of nodes. */
  /* public void testGetGroupOfNodes() { Configuration src =
   * TestHelper.readConfiguration(RESOURCES_LOCATION + "sample_src.txt");
   * Configuration dst = TestHelper.readConfiguration(RESOURCES_LOCATION +
   * "sample_src.txt"); ReconfigurationProblem model =
   * TimedReconfigurationPlanModelHelper.makeBasicModel(src, dst);
   * Assert.assertEquals(model.getNodesGroups().size(), 0); //Create a group
   * ManagedElementSet<Node> grp = new SimpleManagedElementSet<Node>();
   * grp.add(src.getOnlines().get("N1")); grp.add(src.getOnlines().get("N2"));
   * int v = model.getGroup(grp); Assert.assertEquals(v, 0);
   * Assert.assertEquals(model.getNodesGroups().size(), 1); for (Node n : grp) {
   * Assert.assertTrue(model.getAssociatedGroups(n).contains(0));
   * Assert.assertEquals(model.getAssociatedGroups(n).size(), 1); } //Try to
   * create an already defined group grp = new SimpleManagedElementSet<Node>();
   * grp.add(src.getOnlines().get("N1")); grp.add(src.getOnlines().get("N2"));
   * int v2 = model.getGroup(grp); Assert.assertEquals(v2, v);
   * Assert.assertEquals(model.getNodesGroups().size(), 1); for (Node n : grp) {
   * Assert.assertEquals(model.getAssociatedGroups(n).size(), 1); } //A new
   * group with overlapping grp = new SimpleManagedElementSet<Node>();
   * grp.add(src.getOnlines().get("N1")); grp.add(src.getOnlines().get("N3"));
   * int v3 = model.getGroup(grp); Assert.assertEquals(v3, 1);
   * Assert.assertEquals(model.getNodesGroups().size(), 2); Assert.assertEquals
   * (model.getAssociatedGroups(src.getOnlines().get("N1")).size(), 2);
   * Assert.assertEquals((int)
   * model.getAssociatedGroups(src.getOnlines().get("N1")).get(0), 0);
   * Assert.assertEquals((int)
   * model.getAssociatedGroups(src.getOnlines().get("N1")).get(1), 1); } */

  /** Check an instantiation with a non viable source configuration
   * @throws NonViableSourceConfigurationException the expected exception */
  @Test(expectedExceptions = { NonViableSourceConfigurationException.class })
  public void testInstantiationWithNonViableConfiguration()
    ...
    try {
      ...
      Configuration dst = TestHelper.readConfiguration(RESOURCES_LOCATION
          + "sample_src.txt");
      new DefaultReconfigurationProblem(src, dst.getRunnings(),
          dst.getWaitings(), dst.getSleepings(),
          new SimpleManagedElementSet<VirtualMachine>(), dst.getOnlines(),
          dst.getOfflines(), new MockDurationEvaluator(7, 1, 2, 3, 4, 5, 6, 7,
              8));
  ...
  /** Test mustBeRunning on a VM already running. We must observe the addition of
   * a migration. */
  public void testMustBeRunningWithRunning() {
    ...
    ReconfigurationProblem model = TimedReconfigurationPlanModelHelper
        .makeBasicModel(src, dst);
    Assert.assertEquals(model.getAssociatedAction(vm).getClass(),
        MigratableActionModel.class);
  }

  /** Test mustBeRunning on a VM currently sleeping. We must observe the addition
   * of a resume action. */
  public void testMustBeRunningWithSleeping() {
    ...
    ReconfigurationProblem model = TimedReconfigurationPlanModelHelper
        .makeBasicModel(src, dst);
    Assert.assertEquals(model.getAssociatedAction(vm).getClass(),
        ResumeActionModel.class);
  }

  /** Test mustBeRunning on a VM currently waiting. We must observe the addition
   * of a waiting action. */
  public void testMustBeRunningWithWaiting() {
    ...
    ReconfigurationProblem model = TimedReconfigurationPlanModelHelper
        .makeBasicModel(src, dst);
    Assert.assertEquals(model.getAssociatedAction(vm).getClass(),
        RunActionModel.class);
  }

  /** Test mustBeRunning with a VM in an incorrect state.
   * @throws NoAvailableTransitionException the exception we expect */
  /* @Test(expectedExceptions = {NoAvailableTransitionException.class}) public
   * void testMustbeRunningWithUnknownVM() throws PlanException { Configuration
   * src = TestHelper.readConfiguration(RESOURCES_LOCATION + "sample_src.txt");
   * Configuration dst = TestHelper.readConfiguration(RESOURCES_LOCATION +
   * "sample_src.txt"); VirtualMachine vm = new SimpleVirtualMachine("fake", 1,
   * 1, 1); dst.setRunOn(vm, dst.getOnlines().get(0)); try { new
   * DefaultReconfigurationProblem(src, dst.getRunnings(), dst.getWaitings(),
   * dst.getSleepings(), new SimpleManagedElementSet<VirtualMachine>(),
   * dst.getOnlines(), dst.getOfflines(), new MockDurationEvaluator(7, 1, 2, 3,
   * 4, 5, 6, 7, 8)); } catch (MultipleResultingStateException e) {
   * Assert.fail(e.getMessage()); } catch (NonViableSourceConfigurationException
   * e) { Assert.fail(e.getMessage()); } catch (UnknownResultingStateException
   * e) { Assert.fail(e.getMessage()); } } */

  /** Test mustBeSleeping with a running VM. Check the addition of a suspend
   * action. */
  ...
  /** Test mustBeWaiting with a VM in an incorrect state. must return false
  ...
  public void testWithStayOfflines() {
    ...
    src.addOffline(n);
    try {
      ReconfigurationProblem m = new DefaultReconfigurationProblem(src,
          src.getRunnings(), src.getWaitings(), src.getSleepings(),
          new SimpleManagedElementSet<VirtualMachine>(), src.getOnlines(),
          src.getOfflines(), new MockDurationEvaluator(7, 1, 2, 3, 4, 5, 6, 7,
              8));
      Assert.assertEquals(m.getAssociatedAction(n).getClass(),
          StayOfflineNodeActionModel.class);
    }
    catch (Exception e) {
  ...
  public void testStatistics() {
    ...
    try {
      DefaultReconfigurationProblem m = new DefaultReconfigurationProblem(src,
          src.getRunnings(), src.getWaitings(), src.getSleepings(),
          new SimpleManagedElementSet<VirtualMachine>(), src.getOnlines(),
          src.getOfflines(), new MockDurationEvaluator(7, 1, 2, 3, 4, 5, 6, 7,
              8));
      ...
      Assert.assertTrue(st.getNbBacktracks() == 0 && st.getNbNodes() == 0
          && st.getTimeCount() == 0);
      Assert.assertEquals(m.getSolutionsStatistics().size(), 0);
      ISolutionPool sp = SolutionPoolFactory.makeInfiniteSolutionPool(m
          .getSearchStrategy());
  ...
  /** Test the definition of the actions duration with custom boot and halt VM
   * option. */
  public void testWithCustomVMBootAndHalt() {
    Configuration src = new SimpleConfiguration();
    Node n1 = new SimpleNode("N1", 3, 3, 3);
    ...
    vm2.addOption("halt", "30");
    Configuration dst = src.clone();
    dst.remove(vm2);
    ManagedElementSet<VirtualMachine> toHalt = new SimpleManagedElementSet<VirtualMachine>();
    toHalt.add(vm2);
    dst.setRunOn(vm1, n1);
    try {
      ReconfigurationProblem m = new DefaultReconfigurationProblem(src,
          dst.getRunnings(), dst.getWaitings(), dst.getSleepings(), toHalt,
          dst.getOnlines(), dst.getOfflines(), new MockDurationEvaluator(7, 1,
              2, 3, 4, 5, 6, 7, 8));
      ...
      Assert.assertEquals(p.getDuration(), 60);
    }
    catch (Exception e) {
      Assert.fail(e.getMessage(), e);
    }
