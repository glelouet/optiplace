...
public class TestProtobufTimedReconfigurationPlanSerializer {
  ...
  public void test() {
    ...
    ProtobufTimedReconfigurationPlanSerializer s = ProtobufTimedReconfigurationPlanSerializer
        .getInstance();
    File tmpF = null;
    try {
      tmpF = File.createTempFile("out", "out");
      s.write(p, tmpF.getAbsolutePath());
      entropy.plan.ReconfigurationResult r = s.read(tmpF.getAbsolutePath());
