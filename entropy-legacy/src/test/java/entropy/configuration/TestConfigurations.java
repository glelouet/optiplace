...
import static entropy.configuration.ConfigurationUtils.State.Runnings;
import static entropy.configuration.ConfigurationUtils.State.Sleepings;
...
public class TestConfigurations {
  ...
  private Configuration makeConfiguration() {
    ...
    c.setRunOn(new SimpleVirtualMachine("VM3", 1, 1, 1, 2, 2), c.getOnlines()
        .get("N3"));
    c.setRunOn(new SimpleVirtualMachine("VM4", 1, 1, 1),
        c.getOnlines().get("N4"));
    c.setRunOn(new SimpleVirtualMachine("VM5", 1, 1, 1),
        c.getOnlines().get("N5"));
    c.setRunOn(new SimpleVirtualMachine("VM6", 1, 1, 1),
        c.getOnlines().get("N5"));
    c.setRunOn(new SimpleVirtualMachine("VM7", 1, 4, 4, 1, 1), c.getOnlines()
        .get("N5"));
    c.setRunOn(new SimpleVirtualMachine("VM8", 1, 1, 1, 4, 4), c.getOnlines()
        .get("N3"));
    c.setRunOn(new SimpleVirtualMachine("VM9", 1, 1, 1),
        c.getOnlines().get("N8"));
    c.setSleepOn(new SimpleVirtualMachine("VM10", 1, 1, 1),
        c.getOnlines().get("N2"));
    c.setSleepOn(new SimpleVirtualMachine("VM11", 1, 1, 1),
        c.getOnlines().get("N7"));
    return c;
  }
  ...
  public void testUsedNodes() {
    ...
    ManagedElementSet<Node> ns = ConfigurationUtils.usedNodes(cfg,
        EnumSet.of(Runnings));
    Assert.assertEquals(ns.size(), 6);
    ns = ConfigurationUtils.usedNodes(cfg, EnumSet.of(Sleepings));
    Assert.assertEquals(ns.size(), 2);
    ns = ConfigurationUtils.usedNodes(cfg, EnumSet.of(Runnings, Sleepings));
    Assert.assertEquals(ns.size(), 7);
  }

  public void testUnusedNodes() {
    Configuration cfg = makeConfiguration();
    ManagedElementSet<Node> ns = ConfigurationUtils.unusedNodes(cfg, Runnings);
    Assert.assertEquals(ns.size(), 4);
    ns = ConfigurationUtils.unusedNodes(cfg, Sleepings);
    Assert.assertEquals(ns.size(), 8);
  }

  public void testCurrentlyOverloadedNodes() {
    Configuration cfg = makeConfiguration();
    ManagedElementSet<Node> ns = ConfigurationUtils
        .currentlyOverloadedNodes(cfg);
    Assert.assertFalse(ConfigurationUtils.isCurrentlyViable(cfg));
    Assert.assertEquals(ns.size(), 1);
    Assert.assertEquals(ns.get(0).getName(), "N5");
    cfg.remove(cfg.getRunnings().get("VM7"));
    Assert.assertTrue(ConfigurationUtils.isCurrentlyViable(cfg));
  }

  public void testFutureOverloadedNodes() {
    Configuration cfg = makeConfiguration();
    Assert.assertFalse(ConfigurationUtils.isFutureViable(cfg));
    Assert
        .assertEquals(ConfigurationUtils.futureOverloadedNodes(cfg).size(), 1);
    Assert.assertEquals(ConfigurationUtils.futureOverloadedNodes(cfg).get(0)
        .getName(), "N3");
    cfg.remove(cfg.getRunnings().get("VM3"));
    Assert.assertTrue(ConfigurationUtils.isFutureViable(cfg));
  ...
  public void testSubConfiguration() {
    ...
    try {
      Assert.assertEquals(ConfigurationUtils.subConfiguration(cfg,
          cfg.getAllVirtualMachines(), cfg.getAllNodes()), cfg);

      Configuration c = ConfigurationUtils.subConfiguration(cfg, vms, ns);
  ...
  public void testGoodMerge() {
    try {
      Configuration c1 = PlainTextConfigurationSerializer.getInstance().read(
          RESOURCE_ROOT + "testMerge1.txt");
      Configuration c2 = PlainTextConfigurationSerializer.getInstance().read(
          RESOURCE_ROOT + "testMerge2.txt");
      Configuration expected = PlainTextConfigurationSerializer.getInstance()
          .read(RESOURCE_ROOT + "testMerge-good.txt");
      Configuration res = ConfigurationUtils.merge(c1, c2);
      List<Configuration> l = new ArrayList<Configuration>();
      l.add(c1);
      l.add(c2);
      Assert.assertEquals(res, expected);
      Assert.assertEquals(res, ConfigurationUtils.merge(l));
    }
    catch (Exception e) {
      Assert.fail(e.getMessage(), e);
    }
  ...
  public void testBadMergeWithConflictingVMs() throws ConfigurationsException {
    try {
      Configuration c1 = PlainTextConfigurationSerializer.getInstance().read(
          RESOURCE_ROOT + "testMerge1.txt");
      Configuration c2 = PlainTextConfigurationSerializer.getInstance().read(
          RESOURCE_ROOT + "testMerge3.txt");
      ConfigurationUtils.merge(c1, c2);
    }
    catch (IOException e) {
      Assert.fail(e.getMessage(), e);
    }
