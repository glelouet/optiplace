...
/** A Decision module that ask to always restart failed VMs and uses a tendency
 * based approach to estimate the CPU demand of the VMs.
...
public class TendencyBasedDecisionModule extends DecisionModule {
  ...
  public TendencyBasedDecisionModule() {
    preds = new HashMap<VirtualMachine, VMTendencyPredictor>();
  }
    ...
    if (cfg != null) {
      for (VirtualMachine vm : cfg.getAllVirtualMachines()) {
        if (!preds.containsKey(vm)) {
          preds.put(vm, new VMTendencyPredictor(step));
        }
        preds.get(vm).predictCPUDemand(vm);
        if (cfg.isRunning(vm)
            && vm.getCPUDemand() > cfg.getLocation(vm).getCPUCapacity()) {
          int d = vm.getCPUDemand();
          DecisionModule.getLogger().debug(
              vm.getName() + "' demand '" + d + "' decreased to '"
                  + cfg.getLocation(vm).getCPUCapacity() + "'");
