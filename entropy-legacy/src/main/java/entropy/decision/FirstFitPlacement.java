...
public class FirstFitPlacement extends DecisionModule {
  ...
  /** New FirstFitPlacement that use specific sorter for the list of virtual
   * machines and nodes.
   * @param vmCmp The comparator to use to sort the virtual machines. May be
   * null.
   * @param nodeCmp The comparator to use to sort the nodes. May be null. */
  public FirstFitPlacement(VirtualMachineComparator vmCmp,
      NodeComparator nodeCmp) {
    vmComp = vmCmp;
    nodeComp = nodeCmp;
   ...
   * @return true if the CPU capacity of the node is >= the consumption of the
   * virtual machine. Same for the memory requirement and consumption. */
  ...
  /** Assign a virtual machine to a node. The virtual machine must fit to obtain
   * a viable configuration
   * @param conf the configuration
   * @param node the hoster.
   * @param vm the hostee. */
  private static void assignVirtualMachine(Configuration conf, Node node,
      VirtualMachine vm) {
   ...
   * @return a new configuration that respect the CPU and memory requirements of
   * all the virtual machines.
   * @throws AssignmentException if at least one virtual machine can not be
   * assigned to a node */
  @Override
  public Configuration compute(Configuration curConf)
      throws AssignmentException {

    // Deep copy of the nodes
    for (Node n : curConf.getOnlines()) {
      tmpNodes.add(n.clone());
    }
    final ManagedElementSet<VirtualMachine> vms = curConf.getRunnings().clone();

    if (nodeComp != null) {
      Collections.sort(tmpNodes, nodeComp);
    }
    if (vmComp != null) {
      Collections.sort(vms, vmComp);
    }
