...
/** An interface to specify a node and virtual machine builder. This builder is
 * used when a vjob is deserialized. It has to retrieve the existing nodes and
 * virtual machines in a configuration from their name, or ask for a virtual
 * machine instantiation is the virtual machine is not yet handled.
...
public interface VJobElementBuilder {
   ...
   * @return a node if such an element exists in the current configuration with
   * the identifier. {@code null} otherwise */
  Node matchAsNode(String id);

  /** Get an element as an existing virtual machine.
   * @param id the identifier of the virtual machine
   * @return a virtual machine is such an element exists in the current
   * configuraton or {@code null} */
   ...
   * @return if the virtual machine already exists in the current configuration
   * if it matches also the template. In this case, options are overrided.
   * Otherwise, a new virtual machine will be instantiated using the given
   * template and the options. */
  VirtualMachine matchVirtualMachine(String id, String tpl,
