...
public class FenceBuilder implements PlacementConstraintBuilder {
   ...
   * constraint. */
  @Override
  public Fence buildConstraint(List<VJobElement> args)
      throws ConstraintBuilderException {
    if (args.size() != 2) {
      throw new ConstraintBuilderException(this);
    }
    try {
      ManagedElementSet<VirtualMachine> vms = (ManagedElementSet<VirtualMachine>) args
          .get(0);
      ManagedElementSet<Node> ns = (ManagedElementSet<Node>) args.get(1);
