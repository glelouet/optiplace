...
public class PlasmaVJobBuilderBuilder implements VJobBuilderBuilder {
  ...
  /** Build the vjob builder using the default properties file
   * {@link #PROPERTIES}. */
  ...
  @Override
  public PlasmaVJobBuilder build(VJobElementBuilder eb)
      throws VJobBuilderBuilderException {
    try {
      PropertiesHelper propHelper = new PropertiesHelper(file);

      ConstraintsCatalog c = new entropy.vjob.builder.plasma.ConstraintsCatalogBuilderFromProperties(
          propHelper).build();
      PlasmaVJobBuilder b = new PlasmaVJobBuilder(eb, c);
      return b;
    }
    catch (IOException e) {
      throw new VJobBuilderBuilderException(
          "Unable to build the plasma vjob builder:" + e.getMessage(), e);
    }
