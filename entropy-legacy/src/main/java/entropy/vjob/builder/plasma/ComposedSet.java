...
public abstract class ComposedSet<T extends ManagedElement> implements
    VJobSet<T> {
  ...
  public String toString() {
    ...
    /** We use parenthesis when the left or the right operand is a ComposedSet,
     * with a different operator. */
    boolean usePara = false;

    StringBuilder b = new StringBuilder();

    if (leftOperand() instanceof ComposedSet
        && !((ComposedSet<?>) leftOperand()).operator().equals(operator())) {
      usePara = true;
    }
    ...
    usePara = false;
    if (rightOperand() instanceof ComposedSet
        && !((ComposedSet<?>) rightOperand()).operator().equals(operator())) {
      usePara = true;
    }
