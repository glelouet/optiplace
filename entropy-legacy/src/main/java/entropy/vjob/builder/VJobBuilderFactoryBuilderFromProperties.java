...
public class VJobBuilderFactoryBuilderFromProperties implements
    VJobBuilderFactoryBuilder {
  ...
  public VJobBuilderFactory build() throws VJobBuilderFactoryBuilderException {
    ...
    try {
      VJobElementBuilder eb = new DefaultVJobElementBuilder(
          new VirtualMachineTemplateFactoryBuilderFromProperties().build(),
          null);
      ...
      try {
        ...
        VJob.logger.debug("Builders available for file extensions: "
            + f.getManagedExtensions());
      }
      catch (MissingRequiredPropertyException e) {
        throw new VJobBuilderFactoryBuilderException(
            "Unable to build the vjob builder: " + e.getMessage(), e);
      }
        ...
        throw new VJobBuilderFactoryBuilderException(
            "Unable to build the vjob builder '" + fqcn + "' : "
                + e.getMessage(), e);
      }
    }
    catch (IOException e) {
      throw new VJobBuilderFactoryBuilderException("Unable to read '" + file
          + "': " + e.getMessage(), e);
    }
