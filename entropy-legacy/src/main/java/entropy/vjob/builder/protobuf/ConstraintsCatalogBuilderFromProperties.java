...
public final class ConstraintsCatalogBuilderFromProperties implements
    PBConstraintsCatalogBuilder {
  ...
  /** The prefix of each contraint to load. Indicates the FQCN of the
   * PBPlacementConstraintBuilder. */
  ...
  /** Build a catalog.
   * @return the catalog
   * @throws ConstraintsCalalogBuilderException if an error occurred while
   * creating the catalog. */
  @Override
  public PBConstraintsCatalog build() throws ConstraintsCalalogBuilderException {
    ...
    if (props.isDefined(CATALOG_CONTENTS_PROPERTIES)) {
      try {
        ...
        for (String constraint : constraints) {
          String fqcn = props.getRequiredProperty(CONSTRAINT_BUILDER
              + constraint);
          Class<?> cl = getClass().getClassLoader().loadClass(fqcn);
          c.add((PBPlacementConstraintBuilder) cl.newInstance());
        }
        VJob.logger.debug("Available constraints for protobuf vjobs: "
            + c.getAvailableConstraints());
      }
      catch (MissingRequiredPropertyException e) {
        throw new ConstraintsCalalogBuilderException(
            "Unable to build the catalog: " + e.getMessage(), e);
      }
