...
/** Factory to write a vjob into a file. The output format is decided depending
 * on the file extension. By default, the factory provides Xml and protobuf
 * serializer
 * @author Fabien Hermenier */
public final class DefaultVJobFileSerializerFactory implements
    VJobFileSerializerFactory {
  ...
  private DefaultVJobFileSerializerFactory() {
    serializers = new THashMap<String, VJobFileSerializer>();
    add(XmlVJobSerializer.getInstance());
    add(ProtobufVJobSerializer.getInstance());
  }
  ...
  public void add(VJobFileSerializer ser) {
    serializers.put(ser.getFileExtension(), ser);
  }

  @Override
  public void write(VJob vjob, String path) throws IOException {
    String ext = path.substring(path.lastIndexOf('.') + 1, path.length());
    VJobFileSerializer ser = serializers.get(ext);
    if (ser == null) {
      throw new IOException("Unable to decide about the output format for '"
          + path + "'. Availables are " + serializers.keySet());
    }
    ser.write(vjob, path);
  }
