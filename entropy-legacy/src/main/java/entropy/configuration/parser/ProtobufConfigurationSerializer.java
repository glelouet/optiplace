...
/** Serialize and un-serialize a configuration from/to a binary format compatible
 * with Google Protocol buffers.
 * @author Fabien Hermenier */
public final class ProtobufConfigurationSerializer extends
  ...
  public static PBConfiguration.Configuration convert(Configuration cfg) {
    PBConfiguration.Configuration.Builder b = PBConfiguration.Configuration
        .newBuilder();
    ...
    for (Node n : cfg.getOnlines()) {
      ...
      PBConfiguration.Configuration.Hoster.Builder hoster = PBConfiguration.Configuration.Hoster
          .newBuilder();
      hoster.setNode(n2);
      for (VirtualMachine vm : cfg.getRunnings(n)) {
        PBConfiguration.Configuration.Hosted.Builder hosted = PBConfiguration.Configuration.Hosted
            .newBuilder();
        hosted.setState(PBConfiguration.Configuration.HostedVMState.RUNNING);
        PBVirtualMachine.VirtualMachine vm2 = PBVirtualMachineSerializer
            .write(vm);
        hosted.setVm(vm2);
