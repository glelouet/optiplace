...
import entropy.vjob.builder.VJobBuilderFactoryBuilderException;
...
public final class EntropyBuilder {
   ...
   * properties to build Entropy
   * @throws MonitorFactoryException if an error occured while instantiating the
   * Monitor
   * @throws ControlLoopFactoryException if an error occurred while
   * instantiating the control loop */
  public static Entropy buildFromProperties(PropertiesHelper props)
      throws IOException, PropertiesHelperException, MonitorFactoryException,
      ControlLoopFactoryException, VJobBuilderFactoryBuilderException {
    MonitorFactory mf = new MonitorFactory(props);
    VJobBuilderFactory vjobFac = new VJobBuilderFactoryBuilderFromProperties(
        "config/vjobsBuilder.properties").build();
    ControlLoopFactory ctlFactory = new ControlLoopFactory(props);
    Entropy e;
    ControlLoop loop = ctlFactory.makeControlLoop(mf.createMonitor(), vjobFac);
    e = new Entropy(loop);
