...
/** An execution module dedicated to the execution of time bounded Action. All
 * feasible actions are made in parallel. The execution module try to execute
 * new action when a Action is committed.
...
public class TimedReconfigurationExecuter {
  ...
  /** Make a new Execution module for a specific plan. Each action will be
   * executed after adaptation by a specific DriverFactory
   * @param f the DriverFactory to transfrom the action into a driver */
  public TimedReconfigurationExecuter(DriverFactory f) {
    factory = f;
    revDependencies = new HashMap<Action, List<Dependencies>>();
    uncommited = new LinkedList<Action>();
  }

  /** Start the execution of a plan. The method is blocking and ends once all the
   * actions have been performed
   * @param plan the plan to execute */
  public void start(entropy.plan.ReconfigurationResult plan) {
  ...
  /** Associate an action with a driver and start its execution using a
   * {@link Executor}.
  ...
  /** Commit the execution of an action. If the execution of the action succeed.
   * Unlocked actions are started.
  ...
  public void commit(Executor e) {
    ...
    if (e.hasSuceeded()) {
      ...
      logger.error("Reconfiguration aborded: " + e.getException().getMessage(),
          e.getException());
    }
  }

  /** Get all the uncommited actions. A reconfiguration process is terminated
   * once all the actions on a plan has been commited
