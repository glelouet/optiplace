...
public abstract class ControlLoop {
  ...
  /** Log a configuration into a file. If an error occurs, it is logged at the
   * error level
  ...
  public String logConfiguration(Configuration c, Date timeStamp, String suffix) {
    if (logDir != null) {
      String filename = logDir + "/" + DATE_FORMAT.format(timeStamp) + "/"
          + HOUR_FORMAT.format(timeStamp) + "-" + suffix + ".txt";
      try {
        PlainTextConfigurationSerializer.getInstance().write(c, filename);
      }
      catch (Exception e) {
        LOGGER.warn("Unable to store the configuration: " + e.getMessage());
      }
      return filename;
    }
    return null;
